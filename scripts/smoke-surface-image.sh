...
image="deskmate-$service:runtime-smoke"
container="deskmate-$service-runtime-smoke-${GITHUB_RUN_ID:-$$}"
...
if [[ "$service" == "portal" ]]; then
  ...
  docker run -d --name "$container" -p 127.0.0.1::"$container_port" \
    ...
    -e AUTH_CLIENT_ID=deskmate-portal \
    ...
    -e AUTH_EMAIL_FROM="deskmate <no-reply@example.com>" \
