...
check_auth_status() {
  tool="$1"
  output_file="$status_dir/deskmate-\${tool}-auth-status.txt"
...
export async function main(): Promise<void> {
  ...
  const actorId = process.env.GIT_CLI_SMOKE_ACTOR_ID ?? `deskmate-git-cli-smoke-${Date.now()}`;
  ...
  const built = buildApp({
    ...config,
    dataDir: process.env.DATA_DIR ?? mkdtempSync(join(tmpdir(), "deskmate-git-cli-smoke-")),
    harness: "mock",
    sessionStore: "memory",
    runStore: "memory",
  });
  ...
  try {
    ...
    if (process.env.GIT_CLI_SMOKE_DESTROY === "1" && process.env.SPRITES_TOKEN) {
      ...
      const name = spriteScopeName(process.env.SPRITES_NAME_PREFIX ?? "deskmate", `personal:${actorId}`);
