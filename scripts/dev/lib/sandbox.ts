...
export async function resolveSandbox(opts: {
  ...
  if (backend === "local") {
    ...
    const image = opts.baseEnv.LOCAL_SANDBOX_IMAGE || "deskmate-sandbox-local:latest";
  ...
  const env: Record<string, string> = {
    ...
    SPRITES_TOKEN: token,
    SPRITES_NAME_PREFIX: opts.baseEnv.SPRITES_NAME_PREFIX || "deskmatedev",
  };
