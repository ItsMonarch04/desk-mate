...
function worktreeDbName(worktree: string): string {
  return `deskmate_dev_${createHash("sha1").update(worktree).digest("hex").slice(0, 12)}`;
}
...
export async function ensureLocalPostgres(worktree: string, log: (msg: string) => void): Promise<LocalPostgres> {
  const container = process.env.DEV_INSTANCE_POSTGRES_CONTAINER || "deskmate-dev-postgres";
  ...
  const password = process.env.DEV_INSTANCE_POSTGRES_PASSWORD || "deskmate-dev";
  const volume = process.env.DEV_INSTANCE_POSTGRES_VOLUME || "deskmate-dev-postgres-data";
