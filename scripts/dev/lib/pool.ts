...
const DESKMATE_CONFIG_DIR = join(CONFIG_HOME, "deskmate");

export function poolStore(): string {
  if (process.env.DESKMATE_POOL_STORE) return process.env.DESKMATE_POOL_STORE;
  if (process.env.DEV_INSTANCE_POOL_STORE) return process.env.DEV_INSTANCE_POOL_STORE;
  return join(DESKMATE_CONFIG_DIR, "slack-pool");
}

export function liveEnvPath(): string {
  if (process.env.DESKMATE_DEV_ENV) return process.env.DESKMATE_DEV_ENV;
  if (process.env.DEV_INSTANCE_LIVE_ENV) return process.env.DEV_INSTANCE_LIVE_ENV;
  return join(DESKMATE_CONFIG_DIR, "dev.env");
}
