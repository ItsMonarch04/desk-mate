...
const flyServiceCtx = (config: DeskmateConfig, appPrefix: string, deployAppPrefix: string): ServiceCtx => ({
  appPrefix,
  orgId: config.orgId,
  deployAppPrefix,
...
const FLY_RESPONSE = "DESKMATE_LAYER_RESPONSE=";
const FLY_REMOTE_ERROR = "DESKMATE_LAYER_ERROR=";
const FLY_REQUEST_TIMEOUT_MS = 120_000;

function flyRequest(config: DeskmateConfig, method: "GET" | "PUT", body: string): { status: number; body: string } {
...
function deriveToml(ctx: FlyCtx, service: ServiceName): string {
  ...
  const modelEnv: Record<string, string> =
    service === "core"
      ? {
          ...(ctx.config.model ? { AGENT_MODEL: ctx.config.model } : {}),
...
export function derivedTomlFor(config: DeskmateConfig, service: ServiceName, repoRoot: string): string {
...
function flyProviderSecrets(config: DeskmateConfig, workload: string): string[] {
...
const body = "deskmate-storage-probe-" + randomUUID();
const key = "deskmate-health/" + randomUUID();
...
function resolveServices(config: DeskmateConfig, only?: ServiceName[]): ServiceName[] {
...
function buildCtx(
  config: DeskmateConfig,
  configDir: string,
  opts: Pick<FlyUpOpts, "buildFrom" | "buildFromPath">,
): FlyCtx {
  const sourceRoot = opts.buildFrom ? sourceRootFor(configDir, opts.buildFromPath) : undefined;
  if (opts.buildFrom && !sourceRoot) {
    throw new CliError(`--build-from requires a Deskmate checkout containing src/ and deploy/<service>/Dockerfile`);
  }
  const appPrefix = appPrefixOf(config);
  if (!config.flyOrg)
...
export function derivedPluginTomlFor(config: DeskmateConfig, plugin: ResolvedPlugin): string {
...
function unsetDisabledSecurityScreenToken(config: DeskmateConfig, appPrefix: string): void {
...
function unsetDisabledFlyPublisherToken(config: DeskmateConfig, appPrefix: string): void {
...
export async function flyUp(config: DeskmateConfig, configDir: string, opts: FlyUpOpts = {}): Promise<void> {
  ...
  try {
    ...
    header(`deskmate up — ${ctx.appPrefix} (target: fly${runMode}${imageMode})`);
    ...
    note(`   Slack app links: deskmate outputs`);
  } finally {
...
function discoverFlyApps(config: DeskmateConfig, configDir: string): FlyAppDiscovery {
  const appPrefix = appPrefixOf(config);
  const deployAppPrefix = config.deployAppPrefix ?? `${appPrefix}-d`;
...
function flyAppNames(config: DeskmateConfig, configDir: string): string[] {
...
export function flyStatus(config: DeskmateConfig, configDir: string): void {
  const apps = flyAppNames(config, configDir);
  header(`deskmate status — ${appPrefixOf(config)} (target: fly)`);
  if (!which(flyBin())) {
...
export function flyDown(config: DeskmateConfig, configDir: string): void {
  const { apps, unverified } = discoverFlyApps(config, configDir);
  header(`deskmate down — ${appPrefixOf(config)} (target: fly)`);
  note("stopping the deployment by scaling every app to 0 machines (reversible: re-run `deskmate up`).");
  if (!which(flyBin())) {
    note(`flyctl not found — run:`);
...
export function flyPinSandbox(config: DeskmateConfig, image: string, configDir = process.cwd()): void {
  const appPrefix = appPrefixOf(config);
  const app = `${appPrefix}-core`;
  const flyOrg = config.flyOrg ?? "";
  if (!flyOrgApps(flyOrg).has(app)) {
    note(
      `${app} is not deployed in Fly organization ${flyOrg} — no live core to roll; the pin only takes effect from the config's sandbox.image on the next \`deskmate up\``,
    );
    return;
  }
  ...
  try {
    ...
    note(
      `${app} is not running — no live core to roll; the pin only takes effect from the config's sandbox.image on the next \`deskmate up\``,
    );
    return;
  }
  const pinned: DeskmateConfig = { ...config, sandbox: { ...config.sandbox, image } };
...
export function flyRollback(config: DeskmateConfig, configPath: string, to?: string): void {
...
export async function flyDoctor(config: DeskmateConfig, configDir: string, envFile?: string): Promise<void> {
  ...
  for (const workload of [...runnableServices(config.services), ...pluginNames]) {
    ...
    if (!existing) {
      step(`${app}: not created yet — secret checks run after the first \`deskmate up\``);
      continue;
    }
...
export function verifyLocalFlyTokens(config: DeskmateConfig, secrets: ReadonlyMap<string, string>): void {
  const verify = (name: string, args: string[], purpose: string): void => {
    ...
    if (result.status !== 0) {
      throw new CliError(
        `${name} was rejected while checking ${purpose}; mint a fresh scoped token, run \`deskmate secrets push\`, and retry`,
      );
    }
...
export async function flySecretsPush(config: DeskmateConfig, configDir: string, envFile?: string): Promise<void> {
  ...
  if (running.length) {
    ...
    warn("run `deskmate up` to apply them — a plain machine restart does not");
  }
}
