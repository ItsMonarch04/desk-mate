import { awsWorkloadArchitecture, loadConfigAt, sandboxImagePinErrors, type DeskmateConfig } from "../config.ts";
...
export interface HostingProvider {
  ...
  /** Optional `deskmate infra` operations; targets without managed infra omit this. */
  ...
  coordinates(config: DeskmateConfig): { accountOrOrganization?: string; region?: string };
...
const docker: HostingProvider = {
  ...
  createBackend: (ctx) => ({
    ...
    pinSandbox: () => {
      note("sandbox pin recorded; re-run `deskmate up` to restart a local core with it");
    },
  }),
  coordinates: () => ({}),
...
const aws: HostingProvider = {
  ...
  publishSandbox: async (ctx, opts) => {
    if (ctx.config.sandbox?.backend !== "sprites") {
      throw new CliError(
        `this AWS deployment runs Lambda MicroVM sandboxes (sandbox.backend is not "sprites"); use \`deskmate sandbox build\` to validate the layer and \`deskmate infra build-image\` to publish the runtime — or set "sandbox.backend": "sprites" with "sandbox.app" to host sandboxes in an operator-published layer image`,
      );
    }
