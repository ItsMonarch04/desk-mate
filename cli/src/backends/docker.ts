...
import { dockerBasePort, sandboxCoreEnv, securityScreenEnv, type DeskmateConfig } from "../config.ts";
...
const ORG_LABEL_KEY = "deskmate.org";
...
const dockerPrefix = (config: DeskmateConfig): string => `deskmate-${safe(config.orgId)}`;
...
function dockerError(args: string[], message: string): CliError {
  ...
  if (/port is already allocated|address already in use/i.test(message)) {
    hint = `\nhint: a host port is already in use — set DESKMATE_BASE_PORT to a free base port.`;
  }
...
function resolveImage(ctx: DockerCtx, service: ServiceName): string {
  if (ctx.buildFrom) {
    ...
    const tag = `deskmate-${service}:local`;
...
function ensurePostgres(ctx: DockerCtx, dryRun: boolean): string {
  ...
  const url = (password: string): string => `postgres://postgres:${password}@pg:5432/deskmate`;
      ...
      if (!state?.pgPassword) {
        throw new CliError(
          `Postgres volume ${pgVolume(ctx)} exists but its password is unknown (deployment state missing). ` +
            `Set DATABASE_URL to point at it, or 'deskmate down --purge' to recreate it (DESTROYS data).`,
        );
      }
...
export function dockerServiceEnv(config: DeskmateConfig, service: ServiceName): Record<string, string> {
...
function serviceEnv(ctx: DockerCtx, service: ServiceName): Record<string, string> {
  ...
  if (service === "core") {
    ...
    if (config.model) out.AGENT_MODEL = config.model;
...
function warnUnforwardedEnvKeys(ctx: DockerCtx): void {
  ...
  warn(
    `.env keys not forwarded to any container: ${dropped.join(", ")} — only computed secret names are ` +
      `injected. Move non-secret settings to "env.<service>" in the Deskmate deployment config.`,
  );
}
...
export async function dockerUp(
  ...
  header(`deskmate up — ${config.orgId} (target: docker${opts.buildFrom ? ", build-from-source" : ""})`);
...
function printUrls(ctx: DockerCtx): void {
  ...
  note(`   core   : ${url("core")}`);
  note(`   status : deskmate status   ·   logs: deskmate logs core   ·   stop: deskmate down`);
}

export function dockerStatus(config: DeskmateConfig): void {
  requireDocker();
  header(`deskmate status — ${config.orgId}`);
  dockerInherit([
    "ps",
    "-a",
    "--filter",
...
export async function dockerLogs(
  ...
  if (names.length === 0) die(`no containers for ${config.orgId} (is the stack up? run \`deskmate up\`)`);
...
export async function dockerDown(config: DeskmateConfig, opts: { purge?: boolean } = {}): Promise<void> {
  requireDocker();
  const prefix = dockerPrefix(config);
  header(`deskmate down — ${config.orgId}`);
