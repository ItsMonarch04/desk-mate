...
import type { EmailTransport, ModelProvider, DeskmateConfig } from "./config.ts";
...
export interface ProviderScaffold {
  ...
  files(config: DeskmateConfig): ScaffoldFile[];
...
function renderConfig(orgId: string, values: ConfigValues): string {
  ...
  // deployment secret and points the base model at that vendor: \`deskmate setup\`
  // collects the key, \`deskmate doctor\` proves the provider accepts it, and \`deskmate up\`
...
${values.providerFields}
  ...
  // { "name": "linear", "image": "ghcr.io/you/deskmate-linear:1.0" }. A plugin with a
  ...
  // Non-secret env overrides, per service. HARNESS "claude" runs real agent turns
...
2. Run \`npm exec deskmate -- infra render\`, review the result, then run
...
3. Sign-in is handled by the built-in \`auth\` broker: set
   ...
   \`AUTH_ALLOWED_EMAILS\`), then run \`npm exec deskmate -- setup\` for the sender address
...
4. Run \`npm exec deskmate -- infra build-image\` to build the Lambda MicroVM guest image
   and record its immutable version and execution role in the config.
5. Fill the gitignored \`.env\` and run \`npm exec deskmate -- secrets push\`.
6. Run \`npm exec deskmate -- doctor\`, \`npm exec deskmate -- plan\`,
   \`npm exec deskmate -- up --yes\`, and
   \`npm exec deskmate -- check --live\`, in that order.

To tear down this target, run \`npm exec deskmate -- down\`, then persist the destructive
...
\`npm exec deskmate -- infra delete-task-definitions --yes\`, then
\`npm exec deskmate -- infra delete-image --yes\`, then run \`terraform -chdir=infra destroy
-var='ecr_force_delete=true' -var='object_store_force_destroy=true'
-var='db_skip_final_snapshot=true' -var='secret_recovery_window_days=0'\`.
...
export const dockerScaffold: ProviderScaffold = {
  ...
  finalCommand: "npm exec deskmate -- up",
...
`,
      ...
      env: `{ "core": { "HARNESS": "claude", "SNAPSHOT_STORE": "s3", "TRANSFER_STORE": "s3", "S3_BUCKET": ${JSON.stringify(`${orgId}-data`)}, "S3_REGION": "auto" }, "slack": { "SLACK_IDENTITY_EMAIL": "1" }, "auth": { "AUTH_EMAIL_TRANSPORT": ${JSON.stringify(emailTransport)} } }`,
      secretEnv: `,
...
export const gcpScaffold: ProviderScaffold = {
  ...
  "gcp": {
    ...
    "region": "us-central1",
    "artifactRegistry": ${JSON.stringify(`${orgId}-deskmate`)},
    "secretsPrefix": ${JSON.stringify(`${orgId}-deskmate-`)},
    "imageLabel": "latest"
  },
`,
      services: ["core", "slack", "web-ui", "admin", "portal", "auth"],
      env: `{ "core": { "HARNESS": "claude", "SNAPSHOT_STORE": "s3", "TRANSFER_STORE": "s3" }, "slack": { "SLACK_IDENTITY_EMAIL": "1" }, "auth": { "AUTH_EMAIL_TRANSPORT": ${JSON.stringify(emailTransport)} } }`,
      secretEnv: `,
  ...
  // agents execute in, booting the immutable image from \`deskmate sandbox publish\`.
  "sandbox": { "app": ${JSON.stringify(`${orgId}-sandboxes`)} }`,
    }),
  ignores: [
    ".env",
...
2. Fill in the "gcp" block and public URL, run \`npm exec deskmate -- infra render\`,
...
3. Run \`npm exec deskmate -- setup\`, fill the gitignored \`.env\`, and upload it with
   \`npm exec deskmate -- secrets push\`.
4. Publish the Fly Sprites sandbox with \`npm exec deskmate -- sandbox publish\`, then
   run \`npm exec deskmate -- doctor\`, \`npm exec deskmate -- up\`, and
   \`npm exec deskmate -- check --live\`.

Run \`npm exec deskmate -- down\` to remove Cloud Run services. Infrastructure remains
  ...
  renderConfig: (orgId, modelProvider, emailTransport) => {
    ...
    const services = Object.fromEntries(
      [
        ...
        name,
        {
          ecrRepository: `${orgId}-deskmate-${name}`,
...
`,
      services: ["core", "slack", "web-ui", "admin", "portal", "auth"],
      env: `{ "core": { "HARNESS": "claude", "AWS_DEPLOY_IMAGE": ${JSON.stringify(`${cluster}-sandbox`)}, "AWS_PUBLIC_ORIGIN_URL": "http://replace-with-alb-hostname" }, "slack": { "SLACK_IDENTITY_EMAIL": "1" }, "auth": { "AUTH_EMAIL_TRANSPORT": ${JSON.stringify(emailTransport)} } }`,
      secretEnv: `,
  ...
  // (published by \`deskmate infra build-image\`). To boot an operator-published sandbox layer
  // image in a Fly app instead (published by \`deskmate sandbox publish\`), declare it explicitly:
