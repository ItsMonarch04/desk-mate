...
export function acquireAwsLease(aws: AwsConfig, key = "deploy"): AwsLease {
  ...
  try {
    ...
    if (/ConditionalCheckFailedException/.test(errMessage(error))) {
      throw new CliError(
        `another Deskmate operation holds the ${JSON.stringify(key)} lease in ${table}; it expires within ${LEASE_SECONDS / 60} minutes, or delete that item to release it manually`,
      );
    }
    throw error;
  }
  ...
  lease.renewTimer = setInterval(
    ...
    envNum("DESKMATE_AWS_LEASE_RENEW_MS", (LEASE_SECONDS / 3) * 1000),
  );
...
function renewAwsLease(aws: AwsConfig, lease: AwsLease): void {
  ...
  try {
    ...
    if (/ConditionalCheckFailedException/.test(errMessage(error))) {
      ...
      warn(
        `the ${JSON.stringify(lease.key)} lease in ${lease.table} was taken over by another Deskmate operation; this run no longer holds it and a concurrent operation may be mutating the stack`,
      );
    } else {
