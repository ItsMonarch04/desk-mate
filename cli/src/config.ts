...
export const CONFIG_FILENAME = "deskmate.config.jsonc";
...
export function awsWorkloadArchitecture(config: DeskmateConfig, workload: string): "arm64" | "amd64" {
...
export const MODEL_PROVIDER_HARNESSES: Readonly<Record<ModelProvider, readonly string[]>> = {
  anthropic: ["opencode", "claude", "mock"],
  openai: ["opencode", "codex", "mock"],
  openrouter: ["mock"],
};
...
export interface DeskmateConfig {
...
export function securityScreenEnv(config: Pick<DeskmateConfig, "securityScreen">): Record<string, string> {
...
export function loadConfigInDir(
  dir: string,
  overrides?: { target?: Target },
): { config: DeskmateConfig; path: string } {
  const path = configPathInDir(dir);
  if (!path) {
    die(`no ${CONFIG_FILENAME} in ${resolve(dir)} — run \`deskmate init\` there, or pass --config <path>.`);
  }
  return loadConfigAt(path, overrides);
}

export const appPrefixOf = (config: DeskmateConfig): string => config.appPrefix ?? config.orgId;

export const dockerBasePort = (config: DeskmateConfig): number => envNum("DESKMATE_BASE_PORT", config.basePort ?? 8080);

export const isDigestPinned = (ref: string): boolean => /@sha256:[0-9a-f]{64}$/.test(ref);

const SANDBOX_PIN_PENDING = `"sandbox.app" is set but no sandbox layer image is pinned; run \`deskmate sandbox publish\` to build and record the digest-pinned "sandbox.image" agents boot from`;

export const sandboxPinPending = (config: DeskmateConfig): boolean =>
  config.target !== "aws" && Boolean(config.sandbox?.app && !config.sandbox.image);

export function sandboxImagePinErrors(config: DeskmateConfig): Array<{ clause: string; message: string }> {
...
export function loadConfigAt(path: string, overrides?: { target?: Target }): { config: DeskmateConfig; path: string } {
...
function validate(raw: unknown, path: string): DeskmateConfig {
...
function configuredHarness(config: DeskmateConfig): string {
  return config.env.core?.HARNESS?.trim() || (config.target === "fly" ? "claude" : "mock");
}

export function mockHarnessWarning(config: DeskmateConfig): string | undefined {
  if (configuredHarness(config) !== "mock") return undefined;
  const unset = !config.env.core?.HARNESS?.trim();
  return `env.core.HARNESS is ${unset ? "unset, which means" : "set to"} "mock": this deployment answers every message with canned text and calls no model provider. Set it to "claude" for a deployment that runs real agent turns.`;
}

function validateModelProvider(config: DeskmateConfig, path: string): void {
...
function validateBrokerTrust(config: DeskmateConfig, path: string, secrets?: ReadonlyMap<string, string>): void {
...
export function validatePortalTrust(
  config: DeskmateConfig,
  path = "config",
...
function validateAwsFrontDoor(config: DeskmateConfig, path: string): void {
