...
const PLAYBOOKS: Readonly<Record<string, readonly string[]>> = {
  ...
  ADMIN_GRANTS: [
    "Enter the verified email for the first deskmate administrator",
  ...
  FLY_DEPLOY_API_TOKEN: [
    ...
    "A separate Fly organization token for deskmate's per-deployment apps.",
...
export function playbookFor(name: string, config: DeskmateConfig): string[] {
...
export async function runSetup(opts: { dir: string }): Promise<void> {
  ...
  try {
    ...
    if (!configPathInDir(dir)) {
      ...
      note("No Deskmate deployment config here — let's create one.");
      let orgId = "";
    ...
    let n = 1;
    if (remainingRequired.length > 0) stepLine(n++, "deskmate setup", `still missing: ${remainingRequired.join(", ")}`);
    stepLine(n++, "deskmate check", "validate the config and sandbox layer");
    stepLine(n++, "deskmate doctor", "verify external prerequisites read-only");
    if (config.target === "aws")
      stepLine(n++, "see AGENTS.md", "the AWS bootstrap order (Terraform, TLS, portal) before up");
    else stepLine(n++, "deskmate up", "bring the deployment up and print the URLs");
