...
export interface DeploymentOutputs {
  provider: DeskmateConfig["target"];
...
function manifestPath(configDir: string, name: "slack-app-manifest.yml" | "slack-sso-manifest.yml"): string {
  ...
  if (!existsSync(path)) throw new CliError(`${name} is missing; run \`deskmate slack render\``);
  return path;
}

export function deploymentOutputs(config: DeskmateConfig, configDir: string): DeploymentOutputs {
  if (
  ...
  if (bot !== expected.bot) {
    throw new CliError(`Slack manifests do not match the current configuration; run \`deskmate slack render\``);
  }
...
export function runOutputs(config: DeskmateConfig, configDir: string, json: boolean): void {
  ...
  note(`deskmate Slack app: ${output.slack.bot.createUrl}`);
  if (output.slack.sso) {
    note(`deskmate SSO app: ${output.slack.sso.createUrl}`);
    note(`Slack sign-in: ${output.slack.sso.signInUrl}`);
    note(`Slack SSO callback: ${output.slack.sso.redirectUrl}`);
  }
}

export function renderSlackFiles(config: DeskmateConfig, configDir: string): void {
