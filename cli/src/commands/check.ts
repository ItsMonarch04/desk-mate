...
import { mockHarnessWarning, sandboxPinPending, type DeskmateConfig } from "../config.ts";
...
export function runChecks(
  ...
  if (report) {
    ...
    if (sandboxPinPending(config)) {
      warn(
        "no sandbox layer image is pinned yet; run `deskmate sandbox publish` to record one before `deskmate up` renders core",
      );
    }
  }
...
export async function runCheckCommand(
  ...
  header(`deskmate check — ${config.orgId}`);
