...
import { updateConfigSandbox, type DeskmateConfig } from "../config.ts";
...
function prepare(opts: SandboxBuildOpts): PreparedBuild {
  ...
  const dockerfilePath = join(mkdtempSync(join(tmpdir(), "deskmate-sandbox-")), "Dockerfile");
...
export function runSandboxBuild(opts: SandboxBuildOpts): void {
  ...
  header(`deskmate sandbox build → ${tag}`);
  printBuild(prepared);
  if (opts.dryRun) {
...
function publishedRepository(opts: SandboxPublishOpts): string {
  ...
  die("sandbox publish needs sandbox.app in the Deskmate deployment config or --app <registry/repository>.");
}
...
export function runSandboxPublish(opts: SandboxPublishOpts): { image: string } | undefined {
  ...
  const metadataPath = join(mkdtempSync(join(tmpdir(), "deskmate-sandbox-meta-")), "metadata.json");
  const args = [
    "buildx",
    "build",
    "--platform",
  ...
  header(`deskmate sandbox publish → ${tagged}`);
  printBuild(prepared);
  if (opts.dryRun) {
