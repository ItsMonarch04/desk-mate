...
const AGENTS_TEMPLATE = `# Deskmate deployment

This directory is one Deskmate deployment: a config, a secret contract, and a
...
- \`package.json\` pins the exact CLI version this directory is interpreted by,
  so every checkout resolves the same \`deskmate\`. \`contract\` in the config is only
  the coarse compatibility floor; this pin is the reproducible one. Upgrade it
  deliberately and re-run \`deskmate check\` afterwards.
- \`deskmate.config.jsonc\` describes what to run. Every field carries a comment
...
- \`.env.example\` is the secret catalog. It lists every secret the platform
  ...
  \`deskmate init\` creates a gitignored \`.env\`, generates its local signing
  keys, and leaves provider credentials blank for you to fill in. Never write a
  secret value into any other file.
- \`slack-app-manifest.yml\` creates the optional deskmate bot app. Slack OIDC
  deployments also get \`slack-sso-manifest.yml\`. Run
  \`npm exec deskmate -- slack render\` after changing \`publicUrl\`, then
  \`npm exec deskmate -- outputs\` for creation links.
...
1. \`npm exec deskmate -- check\` validates the config and the sandbox layer and prints the
...
2. \`npm exec deskmate -- plan\` reports what deployment would do
   without changing anything.
3. After the target prerequisites are complete, \`npm exec deskmate -- up\` brings the
   ...
   \`--build-from <path to a Deskmate checkout>\` is reserved for contributors
   testing unreleased runtime code.
4. \`npm exec deskmate -- status\`, \`npm exec deskmate -- logs [service]\`, and
   \`npm exec deskmate -- down\` show
   what is running, tail logs, and stop the deployment.
5. \`npm exec deskmate -- secrets push\` uploads the \`.env\` values to the deploy target.
   The docker target reads \`.env\` directly and does not need it.

\`npm exec deskmate -- help\` lists everything else, including \`sandbox build\` and
\`rollback\`.
`;
...
function initialEnv(config: DeskmateConfig): string {
...
function packageContent(dir: string, orgId: string): string {
  ...
  const scripts = {
    deskmate: "deskmate",
    check: "deskmate check",
    plan: "deskmate plan",
    deploy: "deskmate up",
    status: "deskmate status",
    conformance: "deskmate conformance --static",
    ...objectField("scripts"),
  };
  ...
  delete dependencies["deskmate-cli"];
  delete dependencies[packageName];
  for (const group of ["devDependencies", "optionalDependencies"] as const) {
    const groupDependencies = objectField(group);
    if ("deskmate-cli" in groupDependencies || packageName in groupDependencies) {
      delete groupDependencies["deskmate-cli"];
  ...
  const packageJson = {
    ...(exists ? {} : { name: `${orgId}-deskmate-deployment`, version: "0.0.0" }),
    ...existing,
    private: true,
    engines,
    scripts,
...
function scaffoldDeploymentSkill(dir: string): void {
  const files = [
    ...
    ...["fly", "aws", "gcp", "slack", "email"].map((name) => [
      ".codex",
      "skills",
      "deploy-deskmate",
      "references",
...
export function runInit(opts: {
  ...
  step(3, `npm exec deskmate -- setup`, `guided: walk the configured secrets interactively`);
  note(`     (or $EDITOR .env to fill them by hand; local signing keys are generated)`);
  step(4, `npm exec deskmate -- check`, `validate the config and sandbox/`);
