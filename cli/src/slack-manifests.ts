...
export function usesSlackOidc(config: DeskmateConfig): boolean {
...
export function renderSlackManifests(config: DeskmateConfig): SlackManifests {
  const bot = JSON.parse(template("slack-manifest.json")) as {
    display_information: { name: string; description: string };
    features: { bot_user: { display_name: string } };
  };
  bot.display_information.name = "Deskmate";
  bot.display_information.description = `Deskmate workspace agent for ${config.orgId}`;
  bot.features.bot_user.display_name = "Deskmate";
