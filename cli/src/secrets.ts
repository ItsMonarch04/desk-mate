...
import type { ModelProvider, DeskmateConfig } from "./config.ts";
...
type SecretCondition =
  ...
  | { kind: "target"; target: DeskmateConfig["target"] }
...
export const FIRST_PARTY_SECRET_SPECS: readonly SecretSpec[] = [
  ...
  },
  {
    name: "PUBLIC_API_URL",
    service: "core",
    required: { when: { kind: "env-in", service: "core", name: "HARNESS", values: ["opencode", "codex", "claude"] } },
  ...
  },
  {
    ...
    description: "Fly organization token used only by deskmate's opt-in per-deployment app publisher.",
...
function conditionMatches(config: DeskmateConfig, condition: SecretCondition): boolean {
...
function targetEnvDefault(config: DeskmateConfig, service: string, name: string): string | undefined {
  return TARGET_ENV_DEFAULTS[config.target](config, service, name);
}

function requirementFor(config: DeskmateConfig, spec: SecretSpec): boolean | null {
...
export function computedSecrets(config: DeskmateConfig): ComputedSecret[] {
...
function requiresOtherEmailTransport(config: DeskmateConfig, condition: SecretCondition): boolean {
...
export function renderEnvExample(config: DeskmateConfig): string {
  ...
  const lines = [
    ...
    "# the values. .env is gitignored. `deskmate secrets push` transfers values without persisting",
    "# or printing them.",
    "",
  ];
