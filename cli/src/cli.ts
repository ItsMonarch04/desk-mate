...
const CLI_NAME = "deskmate";

const HELP = `${bold(CLI_NAME)} — control-plane CLI for a Deskmate deployment
...
${bold("DEPLOY (operator)")} ${dim("— runs in the deployment directory")}
  ...
  up                                       build images and bring the deployment up
     --build-from[=<deskmate-repo>]              build from local Dockerfiles instead of pulling
  ...
  ${dim("Options (apply to all deploy commands):")}
    --config <path>                        path to deploy config (default: deskmate.config.jsonc in deploy dir)
...
${bold("DEVELOP (contributor)")} ${dim("— runs in the Deskmate repo")}
