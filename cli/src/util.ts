...
export function gitTopLevel(): string {
  try {
    ...
    throw new CliError("not inside a git worktree (run this from a Deskmate checkout)");
  }
}
...
export function resolveBuildRepoRoot(explicit?: string, requiredServices: readonly string[] = ["core"]): string {
  ...
  if (explicit !== undefined) root = resolve(explicit);
    ...
    try {
      ...
      throw new CliError(
        "--build-from requires running inside the Deskmate source repo, or pass a path: --build-from <repo>",
      );
    }
  }
  ...
  if (missing.length) {
    ...
    throw new CliError(
      `--build-from ${source}: not a Deskmate checkout (missing ${missing.map((service) => `deploy/${service}/Dockerfile`).join(", ")})`,
    );
  }
  return root;
}
