...
Never trust the upstream Deskmate repository.
...
terraform -chdir=infra plan -out=deskmate.tfplan
terraform -chdir=infra apply deskmate.tfplan
```

Set `publicUrl`, `env.core.AWS_PUBLIC_ORIGIN_URL`, and `aws.deployRoleArn` from
the Terraform outputs. Finish `npm exec deskmate -- setup .`, render again, and apply.
...
After the first successful deployment, rerun `npm exec deskmate -- up --yes` and
...
scope_key="$(npm exec deskmate -- proof scope-key "$scope_id")"
...
aws --profile <profile> --region <region> s3 cp \
  ...
  tar -xOf - workspace/deskmate-computer-proof.txt
```
...
npm exec deskmate -- rollback --to <release-label-or-manifest-id>
