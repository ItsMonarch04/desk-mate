...
secrets, then run `npm exec deskmate -- setup`, which prompts for exactly the
...
| Enter the values into `.env` through `deskmate setup` | you                                              |
...
discovering it at `deskmate doctor`.
...
4. `deskmate setup` collects it as `RESEND_API_KEY` and the verified sender as
   `AUTH_EMAIL_FROM` (for example `Acme <no-reply@acme.com>`).

`deskmate doctor` calls the Resend API to prove the key is accepted. It cannot prove
...
`deskmate setup` collects `SMTP_HOST`, `SMTP_USERNAME`, and `SMTP_PASSWORD`. Two
...
`deskmate doctor` proves the relay is reachable and answers. It does not authenticate;
...
3. `deskmate setup` values: `SMTP_HOST` is `smtp.gmail.com`, `SMTP_USERNAME` is the
...
3. `deskmate setup` values: `SMTP_HOST` is the region endpoint (for example
...
- `AUTH_ALLOWED_EMAILS` in `.env` for named addresses — `deskmate setup` derives it
