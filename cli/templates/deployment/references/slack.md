...
| App            | Job                                                          | Enabled by                     |
| -------------- | ------------------------------------------------------------ | ------------------------------ |
| `deskmate`     | the agent in channels, DMs, and group messages               | `"slack"` in `services`        |
| `deskmate SSO` | signs people in to the web surfaces with their Slack account | Slack OIDC instead of `"auth"` |
...
Deskmate uses one private Socket Mode app per deployment and workspace.

Run `npm exec deskmate -- outputs` and open the exact bot manifest creation URL.
...
without them — write them anyway. `deskmate slack render` and `deskmate outputs` decide a
...
`deskmate outputs` requires the `slack` service even when only the SSO app is wanted.
...
`outputs` prints `deskmate SSO app` (the manifest creation URL), `Slack sign-in`
...
`.env`, then run `npm exec deskmate -- secrets push`.
