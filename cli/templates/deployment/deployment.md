...
Before cloud mutation, read `deskmate.config.jsonc` when it exists. Its `target` is
...
- model provider: Anthropic, OpenAI, or OpenRouter (one key that routes to
  ...
  `modelProvider` in `deskmate.config.jsonc`, which makes that provider's API key a
...
For a repository without `deskmate.config.jsonc`, first confirm the hosting target
...
`.codex/skills/deploy-deskmate/references/email.md` before collecting secrets — the
...
`.codex/skills/deploy-deskmate/references/slack.md`, which covers the SSO app, the
...
configured. Enable it in `deskmate.config.jsonc`:
...
same pass. `modelProvider` decides which one `deskmate setup` asks for —
...
create the key; you only place it. It is a required secret, so `deskmate doctor` calls
the provider to prove the key is accepted and `deskmate up` refuses a deployment that
...
`openrouter/auto`. Set `model` in `deskmate.config.jsonc` only to override that, and
...
npm exec deskmate -- conformance
npm exec deskmate -- outputs --json
```
...
place a deployment key belongs, and `deskmate secrets push` moves it without printing
it.
...
`/root/workspace/deskmate-computer-proof.txt`, then use the provider reference's
...
deferred. Otherwise read `.codex/skills/deploy-deskmate/references/slack.md`, then run:

```bash
npm exec deskmate -- slack render
npm exec deskmate -- outputs
```
...
- `npm exec deskmate -- status`, logs, rollback, and teardown commands;
