...
process.env.DESKMATE_AWS_ROLLOUT_POLL_MS = "5";
process.env.DESKMATE_AWS_LIVE_PROBE_POLL_MS = "5";
process.env.DESKMATE_AWS_LIVE_PROBE_DEADLINE_MS = "20";
process.env.DESKMATE_AWS_DB_SNAPSHOT_POLL_MS = "5";
...
function fakeAws(
  ...
  const tgName = (name: "core" | "portal"): string =>
    ...
    `acme-deskmate-${name.replaceAll("-", "").slice(0, 4)}-${createHash("sha1").update(`acme-deskmate:${name}`).digest("hex").slice(0, 6)}`;
...
const config: DeskmateConfig = {
  ...
  env: {
    core: { HARNESS: "claude", AWS_DEPLOY_IMAGE: "acme-deskmate-sandbox", AWS_DEPLOY_IMAGE_VERSION: "1" },
...
test("AWS environment derives identity, public URLs, private wiring, and MicroVM coordinates", () => {
  ...
  const microvm = serviceEnvironment(rest as DeskmateConfig, "core");
  assert.equal(microvm.SANDBOX_BACKEND, "aws");
  assert.equal(microvm.AWS_SANDBOX_REGION, "us-west-2");
  assert.equal(microvm.AWS_SANDBOX_IMAGE, "acme-deskmate-sandbox");
  assert.equal(microvm.AWS_SANDBOX_IMAGE_VERSION, "1");
  assert.equal(microvm.AWS_SANDBOX_EXEC_ROLE_ARN, "arn:aws:iam::123456789012:role/acme-deskmate-microvm-exec");
...
test("AWS routes security screen proxy configuration and its token only to core", () => {
  ...
  const image = `123456789012.dkr.ecr.us-west-2.amazonaws.com/deskmate-core@sha256:${"a".repeat(64)}`;
...
test("AWS fixes every workload listen port to its ECS port mapping", () => {
  const configured: DeskmateConfig = {
    ...config,
    env: {
      ...config.env,
...
test("AWS core receives the configured model and virtual-service environment", () => {
  ...
  assert.equal(core.AGENT_MODEL, "anthropic/claude-sonnet-4-5");
...
test("AWS required runtime settings cannot be overridden by config env", () => {
  const overridden: DeskmateConfig = {
    ...config,
    env: {
      ...config.env,
      core: {
...
test("githubTrustSubject pins the deploy branch, never the working checkout's branch", () => {
  const dir = mkdtempSync(join(tmpdir(), "deskmate-aws-trust-subject-"));
  const git = (...args: string[]): void => {
    const result = spawnSync("git", ["-C", dir, ...args], { encoding: "utf8" });
    assert.equal(result.status, 0, result.stderr);
  };
  git("init", "-b", "feature-checkout");
  git("remote", "add", "origin", "git@github.com:acme/deskmate.git");
...
test("AWS deploy refuses the HTTP bootstrap before any AWS mutation", async () => {
  const dir = mkdtempSync(join(tmpdir(), "deskmate-aws-http-bootstrap-"));
...
test("AWS deploy requires the live ALB listener to match the HTTPS public URL", async () => {
  const dir = mkdtempSync(join(tmpdir(), "deskmate-aws-live-listener-"));
  const fake = fakeAws(dir, `console.log("");`);
...
test("AWS deploy binds its public origin DNS to this stack's ALB", async () => {
  const dir = mkdtempSync(join(tmpdir(), "deskmate-aws-dns-binding-"));
...
test("AWS deploy requires the exact active successful MicroVM image version", async () => {
  const dir = mkdtempSync(join(tmpdir(), "deskmate-aws-image-version-"));
  const fake = statefulAws(dir, oneServiceConfig());
  ...
  try {
    ...
    assert.match(
      calls,
      /lambda-microvms get-microvm-image --image-identifier arn:aws:lambda:us-west-2:123456789012:microvm-image:acme-deskmate-sandbox/,
    );
    assert.match(
      calls,
      /lambda-microvms list-microvm-image-versions --image-identifier arn:aws:lambda:us-west-2:123456789012:microvm-image:acme-deskmate-sandbox/,
    );
    assert.doesNotMatch(calls, /dynamodb put-item|ecr get-login-password|ecs update-service/);
  } finally {
...
test("AWS deploy requires the live core-only ALB to default 404 and expose exactly /v1/*", async () => {
  const dir = mkdtempSync(join(tmpdir(), "deskmate-aws-live-routing-"));
  const run = async (
    env:
...
test("AWS portal ALB adopts pinned target groups and requires exactly the env-derived host rules", async () => {
  const dir = mkdtempSync(join(tmpdir(), "deskmate-aws-host-split-"));
  ...
  const hostSplitConfig = (hosts: { apiUrl?: string; appsDomain?: string }): DeskmateConfig => ({
    ...config,
...
test("AWS up scales services to the configured desired count and live check flags drift from it", async () => {
  const dir = mkdtempSync(join(tmpdir(), "deskmate-aws-desired-count-"));
  const dockerBin = join(dir, "docker");
  writeFileSync(dockerBin, `#!/usr/bin/env node\nconsole.log("Digest: sha256:${"a".repeat(64)}");\n`);
  chmodSync(dockerBin, 0o755);
  const scaled = (): DeskmateConfig => {
...
test("AWS up snapshots the database under the lease before any mutation and records it in the manifest", async () => {
  const dir = mkdtempSync(join(tmpdir(), "deskmate-aws-db-snapshot-"));
  const dockerBin = join(dir, "docker");
  writeFileSync(dockerBin, `#!/usr/bin/env node\nconsole.log("Digest: sha256:${"a".repeat(64)}");\n`);
  chmodSync(dockerBin, 0o755);
  ...
  try {
    ...
    const health = calls.indexOf("rds describe-db-instances --db-instance-identifier acme-deskmate-core");
    ...
    assert.equal(
      manifest.dbSnapshot,
      `acme-deskmate-core-predeploy-${manifestId}`,
...
test("AWS up refuses to mutate when the database is unavailable or keeps no automated backups", async () => {
  const dir = mkdtempSync(join(tmpdir(), "deskmate-aws-db-unhealthy-"));
  const single = oneServiceConfig();
  ...
  try {
    ...
    await assert.rejects(
      ...
      /database acme-deskmate-core is backing-up; refusing to deploy/,
    );
    ...
    const strict: DeskmateConfig = { ...single, aws: { ...single.aws!, dbRetentionMinDays: 35 } };
    await assert.rejects(
...
test("aws.predeployDbSnapshot false skips the snapshot without touching RDS", async () => {
  const dir = mkdtempSync(join(tmpdir(), "deskmate-aws-db-disabled-"));
  const dockerBin = join(dir, "docker");
  writeFileSync(dockerBin, `#!/usr/bin/env node\nconsole.log("Digest: sha256:${"a".repeat(64)}");\n`);
  chmodSync(dockerBin, 0o755);
  const base = oneServiceConfig();
  const single: DeskmateConfig = { ...base, aws: { ...base.aws!, predeployDbSnapshot: false } };
  const fake = statefulAws(dir, single);
  const priorPath = process.env.PATH;
  process.env.PATH = `${dir}:${priorPath}`;
  try {
...
test("AWS up prunes pre-deploy snapshots beyond the bound, legacy untagged ones included, and never manifest-referenced, foreign, or operator snapshots", async () => {
  const dir = mkdtempSync(join(tmpdir(), "deskmate-aws-db-prune-"));
  const dockerBin = join(dir, "docker");
  writeFileSync(dockerBin, `#!/usr/bin/env node\nconsole.log("Digest: sha256:${"a".repeat(64)}");\n`);
  chmodSync(dockerBin, 0o755);
  const single = oneServiceConfig();
  const clusterTag = [
    { Key: "ManagedBy", Value: "deskmate-cli" },
    { Key: "DeskmateCluster", Value: "acme-deskmate" },
  ];
      ...
      [
        {
          id: "current",
          imageLabel: "release",
          dbSnapshot: "acme-deskmate-core-predeploy-oldest",
  ...
  state.rdsSnapshots = [
    {
      DBSnapshotIdentifier: "acme-deskmate-core-predeploy-oldest",
      Status: "available",
      SnapshotCreateTime: "2026-01-01T00:00:00.000Z",
      TagList: clusterTag,
    },
    {
      DBSnapshotIdentifier: "acme-deskmate-core-predeploy-older",
      Status: "available",
      SnapshotCreateTime: "2026-01-02T00:00:00.000Z",
      TagList: clusterTag,
    },
    {
      DBSnapshotIdentifier: "acme-deskmate-core-predeploy-newest",
      Status: "available",
      SnapshotCreateTime: "2026-01-03T00:00:00.000Z",
      TagList: clusterTag,
    },
    { DBSnapshotIdentifier: "acme-deskmate-core-predeploy-aborted", Status: "creating", TagList: clusterTag },
    {
      DBSnapshotIdentifier: "acme-deskmate-core-predeploy-legacy",
      Status: "available",
      SnapshotCreateTime: "2026-01-01T12:00:00.000Z",
      TagList: [{ Key: "ManagedBy", Value: "deskmate-cli" }],
    },
    {
      DBSnapshotIdentifier: "acme-deskmate-core-predeploy-foreign",
      Status: "available",
      SnapshotCreateTime: "2026-01-01T00:00:00.000Z",
      TagList: [
        { Key: "ManagedBy", Value: "deskmate-cli" },
        { Key: "DeskmateCluster", Value: "other-deskmate" },
      ],
    },
    {
      DBSnapshotIdentifier: "acme-deskmate-core-operator-kept",
      Status: "available",
  ...
  writeFileSync(fake.state, JSON.stringify(state));
  const priorPath = process.env.PATH;
  const priorKeep = process.env.DESKMATE_AWS_DB_SNAPSHOT_KEEP;
  process.env.PATH = `${dir}:${priorPath}`;
  process.env.DESKMATE_AWS_DB_SNAPSHOT_KEEP = "2";
  try {
    await awsUp(single, dir, { yes: true });
    ...
    assert.deepEqual(
      kept,
      [
        "acme-deskmate-core-operator-kept",
        "acme-deskmate-core-predeploy-aborted",
        "acme-deskmate-core-predeploy-foreign",
        "acme-deskmate-core-predeploy-oldest",
        `acme-deskmate-core-predeploy-${manifestId}`,
      ].sort(),
    );
  } finally {
    process.env.PATH = priorPath;
    if (priorKeep === undefined) delete process.env.DESKMATE_AWS_DB_SNAPSHOT_KEEP;
    else process.env.DESKMATE_AWS_DB_SNAPSHOT_KEEP = priorKeep;
    fake.restore();
    rmSync(dir, { recursive: true, force: true });
  }
});
...
test("AWS up fails closed when the snapshot reports an unexpected status or never becomes available", async () => {
  const single = oneServiceConfig();
  const brokenDir = mkdtempSync(join(tmpdir(), "deskmate-aws-db-broken-"));
  ...
  const stuckDir = mkdtempSync(join(tmpdir(), "deskmate-aws-db-stuck-"));
  const stuck = statefulAws(stuckDir, single, {}, { snapshotCreatingPolls: 10_000 });
  const priorDeadline = process.env.DESKMATE_AWS_DB_SNAPSHOT_DEADLINE_MS;
  process.env.DESKMATE_AWS_DB_SNAPSHOT_DEADLINE_MS = "40";
  try {
    await assert.rejects(
    ...
    if (priorDeadline === undefined) delete process.env.DESKMATE_AWS_DB_SNAPSHOT_DEADLINE_MS;
    else process.env.DESKMATE_AWS_DB_SNAPSHOT_DEADLINE_MS = priorDeadline;
    stuck.restore();
...
test("a snapshot prune failure warns without failing the deploy", async () => {
  const dir = mkdtempSync(join(tmpdir(), "deskmate-aws-db-prune-warn-"));
  const dockerBin = join(dir, "docker");
  writeFileSync(dockerBin, `#!/usr/bin/env node\nconsole.log("Digest: sha256:${"a".repeat(64)}");\n`);
  chmodSync(dockerBin, 0o755);
  const single = oneServiceConfig();
  ...
  state.rdsSnapshots = [
    {
      DBSnapshotIdentifier: "acme-deskmate-core-predeploy-stale",
      Status: "available",
      SnapshotCreateTime: "2026-01-01T00:00:00.000Z",
      TagList: [{ Key: "DeskmateCluster", Value: "acme-deskmate" }],
    },
  ];
  writeFileSync(fake.state, JSON.stringify(state));
  const priorPath = process.env.PATH;
  const priorKeep = process.env.DESKMATE_AWS_DB_SNAPSHOT_KEEP;
  process.env.PATH = `${dir}:${priorPath}`;
  process.env.DESKMATE_AWS_DB_SNAPSHOT_KEEP = "1";
  ...
  try {
    ...
    assert.match(
      warnings.join("\n"),
      /could not prune pre-deploy database snapshot acme-deskmate-core-predeploy-stale/,
    );
    const after = JSON.parse(readFileSync(fake.state, "utf8"));
...
test("the snapshot wait tolerates transient describe failures and fails after three in a row", async () => {
  const single = oneServiceConfig();
  const transientDir = mkdtempSync(join(tmpdir(), "deskmate-aws-db-transient-"));
  ...
  const brokenDir = mkdtempSync(join(tmpdir(), "deskmate-aws-db-describe-broken-"));
...
test("a no-op re-deploy deletes its unreferenced pre-deploy snapshot and records no manifest", async () => {
  const dir = mkdtempSync(join(tmpdir(), "deskmate-aws-db-noop-"));
  const dockerBin = join(dir, "docker");
  writeFileSync(dockerBin, `#!/usr/bin/env node\nconsole.log("Digest: sha256:${"a".repeat(64)}");\n`);
  chmodSync(dockerBin, 0o755);
  const single = oneServiceConfig();
  ...
  try {
    ...
    assert.deepEqual(
      ...
      [`acme-deskmate-core-predeploy-${firstManifestId}`],
...
test("AWS up renews the deploy lease with a holder-conditioned update while it runs", async () => {
  const dir = mkdtempSync(join(tmpdir(), "deskmate-aws-lease-renew-"));
  const dockerBin = join(dir, "docker");
  writeFileSync(dockerBin, `#!/usr/bin/env node\nconsole.log("Digest: sha256:${"a".repeat(64)}");\n`);
  chmodSync(dockerBin, 0o755);
  const single = oneServiceConfig();
  ...
  const priorPath = process.env.PATH;
  const priorRenew = process.env.DESKMATE_AWS_LEASE_RENEW_MS;
  process.env.PATH = `${dir}:${priorPath}`;
  process.env.DESKMATE_AWS_LEASE_RENEW_MS = "10";
  try {
    await awsUp(single, dir, { yes: true });
    const calls = readFileSync(fake.log, "utf8");
    assert.match(
      calls,
      /dynamodb update-item --table-name acme-deskmate-deploy-locks --key \{"lockKey":\{"S":"deploy"\}\} --update-expression SET expiresAt = :expiresAt --condition-expression holder = :holder/,
    );
    assert.ok(
    ...
    process.env.PATH = priorPath;
    if (priorRenew === undefined) delete process.env.DESKMATE_AWS_LEASE_RENEW_MS;
    else process.env.DESKMATE_AWS_LEASE_RENEW_MS = priorRenew;
    fake.restore();
    rmSync(dir, { recursive: true, force: true });
  }
});
...
test("a lease renewal that loses the holder condition warns loudly and stops renewing", async () => {
  const dir = mkdtempSync(join(tmpdir(), "deskmate-aws-lease-lost-"));
  const fake = fakeAws(
    dir,
    `
...
console.log("");`,
  ...
  const priorRenew = process.env.DESKMATE_AWS_LEASE_RENEW_MS;
  process.env.DESKMATE_AWS_LEASE_RENEW_MS = "5";
  const warnings: string[] = [];
  const warnLog = console.warn;
  console.warn = (...parts: unknown[]): void => void warnings.push(parts.join(" "));
  try {
    ...
    assert.match(
      warnings.join("\n"),
      /"deploy" lease in acme-deskmate-deploy-locks was taken over by another Deskmate operation/,
    );
    assert.equal(
...
test("AWS deploy requires PUBLIC_API_URL to equal the declared HTTPS public URL", async () => {
  const dir = mkdtempSync(join(tmpdir(), "deskmate-aws-public-api-url-"));
  const fake = statefulAws(dir, oneServiceConfig());
...
test("every AWS mutation rejects the wrong caller account before side effects", async () => {
  const dir = mkdtempSync(join(tmpdir(), "deskmate-aws-account-guard-"));
  const fake = fakeAws(dir, `console.log("");`);
...
test("AWS deploy rejects required secret containers without an AWSCURRENT value before mutation", async () => {
  const dir = mkdtempSync(join(tmpdir(), "deskmate-aws-empty-secret-"));
  const targetName = `acme-deskmate-port-${createHash("sha1").update("acme-deskmate:portal").digest("hex").slice(0, 6)}`;
...
test("AWS deploy rejects weak signing keys before mutation", async () => {
  const dir = mkdtempSync(join(tmpdir(), "deskmate-aws-weak-secret-"));
  const fake = statefulAws(dir, oneServiceConfig());
...
test("AWS doctor verifies durable deployment storage and object-store access", () => {
  const dir = mkdtempSync(join(tmpdir(), "deskmate-aws-doctor-storage-"));
  const ready = fakeAws(
    dir,
    `
if (a.includes("dynamodb describe-table")) console.log(JSON.stringify({ Table: { TableStatus: "ACTIVE" } }));
  ...
  try {
    ...
    assert.match(calls, /dynamodb describe-table --table-name acme-deskmate-deploy-locks/);
    assert.ok(
      calls.includes(
...
test("AWS task definitions are digest-pinned and route only computed secrets", () => {
  ...
  assert.equal(task.executionRoleArn, "arn:aws:iam::123456789012:role/acme-deskmate-task-execution");
  assert.equal(task.taskRoleArn, "arn:aws:iam::123456789012:role/acme-deskmate-core-task");
  const container = task.containerDefinitions[0]!;
...
test("AWS task architecture allows per-workload overrides", () => {
  const amd64Core: DeskmateConfig = {
    ...config,
    aws: {
      ...config.aws!,
      services: {
  ...
  const coreImage = `123456789012.dkr.ecr.us-west-2.amazonaws.com/deskmate-core@sha256:${"a".repeat(64)}`;
...
test("AWS source builds honor the configured web-ui base build-arg and record git provenance", async () => {
  const dir = mkdtempSync(join(tmpdir(), "deskmate-aws-web-ui-build-"));
  ...
  const priorPath = process.env.PATH;
  process.env.PATH = `${dir}:${priorPath}`;
  const webUiConfig: DeskmateConfig = {
    ...config,
  ...
  try {
    ...
    const build = readFileSync(dockerLog, "utf8")
      .split("\n")
      .find((line) => line.includes("buildx build") && line.includes("deskmate-web-ui"));
    ...
    const changedPrebuiltConfig: DeskmateConfig = {
      ...webUiConfig,
...
test("AWS source builds honor a per-service dockerfile override and stamp GIT_SHA, suffixed -dirty on a dirty checkout", async () => {
  const dir = mkdtempSync(join(tmpdir(), "deskmate-aws-dockerfile-override-"));
  const sourceDir = join(dir, "source");
  const dockerLog = join(dir, "docker.log");
  const dockerBin = join(dir, "docker");
  writeFileSync(dockerLog, "");
  ...
  git("commit", "-m", "initial");
  const head = git("rev-parse", "HEAD");
  const layeredConfig: DeskmateConfig = {
    ...config,
    services: ["core", "web-ui", "portal"],
    aws: {
      ...config.aws!,
  ...
  try {
    ...
    const coreBuild = builds.find((line) => line.includes("deskmate-core"));
    assert.ok(
    ...
    const webUiBuild = builds.find((line) => line.includes("deskmate-web-ui"));
    assert.ok(
    ...
    const dirtyCoreBuild = readFileSync(dockerLog, "utf8")
      .split("\n")
      .find((line) => line.includes("buildx build") && line.includes("deskmate-core") && line.includes(dirtySource));
    assert.ok(
    ...
    const missingConfig: DeskmateConfig = {
      ...layeredConfig,
      aws: {
        ...layeredConfig.aws!,
        services: {
...
test("AWS source-plugin provenance records the build source and detects source-mode drift", async () => {
  const dir = mkdtempSync(join(tmpdir(), "deskmate-aws-plugin-provenance-"));
  ...
  const single = oneServiceConfig();
  const sourceConfig: DeskmateConfig = {
    ...single,
    plugins: [{ name: "linear" }],
    aws: {
      ...single.aws!,
  ...
  try {
    ...
    const configuredImage: DeskmateConfig = {
      ...sourceConfig,
...
test("AWS live-image provenance accepts only the configured ECR repository and a full digest", () => {
  ...
  assert.equal(
    isPinnedWorkloadImage(config, "core", "123456789012.dkr.ecr.us-west-2.amazonaws.com/deskmate-core:latest"),
    false,
  );
});

test("AWS renders third-party plugins as private ECS workloads with scoped secrets", () => {
  const pluginConfig: DeskmateConfig = {
    ...config,
    plugins: [
      {
        name: "linear",
  ...
  const image = `123456789012.dkr.ecr.us-west-2.amazonaws.com/deskmate-linear@sha256:${"c".repeat(64)}`;
...
test("AWS task rendering rejects guessed architecture for external images", () => {
  ...
  const overriddenCore: DeskmateConfig = { ...config, imageOverrides: { core: "ghcr.io/acme/core:1" } };
  const coreImage = `123456789012.dkr.ecr.us-west-2.amazonaws.com/deskmate-core@sha256:${"a".repeat(64)}`;
  assert.throws(() => renderTaskDefinition(overriddenCore, "core", coreImage), /core\.architecture is required/);

  const sourcePlugin: DeskmateConfig = { ...pluginConfig, plugins: [{ name: "linear" }] };
...
test("AWS secret upload reads the deployment .env and removes every plaintext staging file", async () => {
  const dir = mkdtempSync(join(tmpdir(), "deskmate-aws-secrets-"));
  const bin = join(dir, "aws-fake");
...
test("AWS secret upload rejects active services when no deployment manifest exists", async () => {
  const dir = mkdtempSync(join(tmpdir(), "deskmate-aws-secrets-active-"));
  const secretsConfig: DeskmateConfig = { ...oneServiceConfig(), env: {} };
...
test("AWS secret rotation holds the deploy lease across the complete write set", async () => {
  const dir = mkdtempSync(join(tmpdir(), "deskmate-aws-secret-lease-"));
  const secretsConfig: DeskmateConfig = { ...oneServiceConfig(), env: {} };
  const operator = computedSecrets(secretsConfig).filter(
    (secret) => secret.managedBy === "operator" && secret.required,
  );
  ...
  try {
    ...
    assert.match(calls, /ecs update-service --cluster acme-deskmate --service acme-core --force-new-deployment/);
...
test("AWS secret rotation refuses foreign exact-name services before uploading or restarting", async () => {
  const dir = mkdtempSync(join(tmpdir(), "deskmate-aws-foreign-secret-rotation-"));
  const secretsConfig: DeskmateConfig = { ...oneServiceConfig(), env: {} };
  const operator = computedSecrets(secretsConfig).filter(
    (secret) => secret.managedBy === "operator" && secret.required,
  );
...
test("AWS secret upload registers and records a task revision for a newly supplied optional secret", async () => {
  const dir = mkdtempSync(join(tmpdir(), "deskmate-aws-optional-secret-"));
...
test("AWS optional-secret activation restores prior tasks when a later service restart fails", async () => {
  const dir = mkdtempSync(join(tmpdir(), "deskmate-aws-secret-compensation-"));
...
const oneServiceConfig = (name = "core"): DeskmateConfig => ({
  ...config,
  services: ["core"],
  aws: { ...config.aws!, services: { [name]: config.aws!.services[name] ?? config.aws!.services.core! } },
});

const twoServiceConfig = (): DeskmateConfig => ({
  ...config,
...
test("rollback uses a coherent durable manifest across independent service histories", async () => {
  const dir = mkdtempSync(join(tmpdir(), "deskmate-aws-rollback-"));
...
test("AWS rollback surfaces the pre-deploy database snapshot of the deployment it rolls back", async () => {
  const dir = mkdtempSync(join(tmpdir(), "deskmate-aws-rollback-snapshot-"));
  const single = oneServiceConfig();
  const fake = statefulAws(
    dir,
    single,
    manifestItems(
      [
        { id: "old", tasks: { core: "arn:aws:ecs:us-west-2:123456789012:task-definition/acme-core:7" } },
        {
          id: "current",
          previous: "old",
          dbSnapshot: "acme-deskmate-core-predeploy-current",
  ...
  try {
    ...
    assert.match(
      out,
      /restore-db-instance-from-db-snapshot --db-snapshot-identifier acme-deskmate-core-predeploy-current --db-instance-identifier acme-deskmate-core-restored --region us-west-2, then repoint the stack at the restored instance/,
    );
  } finally {
    console.log = log;
    fake.restore();
...
test("AWS rollback --to a manifest several steps back surfaces the target's successor snapshot, not the current one", async () => {
  const dir = mkdtempSync(join(tmpdir(), "deskmate-aws-rollback-snapshot-chain-"));
  const single = oneServiceConfig();
  const fake = statefulAws(
    dir,
    single,
    manifestItems(
      [
        { id: "a", tasks: { core: "arn:aws:ecs:us-west-2:123456789012:task-definition/acme-core:5" } },
        {
          id: "b",
          previous: "a",
          dbSnapshot: "acme-deskmate-core-predeploy-b",
          tasks: { core: "arn:aws:ecs:us-west-2:123456789012:task-definition/acme-core:7" },
        },
        {
          id: "c",
          previous: "b",
          dbSnapshot: "acme-deskmate-core-predeploy-c",
          tasks: { core: "arn:aws:ecs:us-west-2:123456789012:task-definition/acme-core:9" },
        },
      ],
      "c",
  ...
  try {
    ...
    assert.match(
      out,
      /--db-snapshot-identifier acme-deskmate-core-predeploy-b --db-instance-identifier acme-deskmate-core-restored --region us-west-2/,
      "the data restore point is the snapshot taken before the target's successor",
    );
    assert.doesNotMatch(out, /acme-deskmate-core-predeploy-c/);
  } finally {
    console.log = log;
    fake.restore();
    rmSync(dir, { recursive: true, force: true });
...
test("AWS rollback surfaces the snapshot even when a rotation manifest sits directly after the target", async () => {
  const dir = mkdtempSync(join(tmpdir(), "deskmate-aws-rollback-snapshot-rotation-"));
  const single = oneServiceConfig();
  const fake = statefulAws(
    dir,
    single,
  ...
  try {
    ...
    assert.match(
      lines.join("\n"),
      /--db-snapshot-identifier acme-deskmate-core-predeploy-c/,
...
test("rollback resolves a label through its full-service manifest before mutating ECS", async () => {
  const dir = mkdtempSync(join(tmpdir(), "deskmate-aws-rollback-label-"));
  const single = oneServiceConfig();
...
test("AWS rollback restores the recorded layer without reading the broken current data plane", async () => {
  const dir = mkdtempSync(join(tmpdir(), "deskmate-aws-rollback-layer-"));
  const single = oneServiceConfig();
...
test("AWS rollback of a scaled-to-zero stack defers the layer sync instead of failing", async () => {
  const dir = mkdtempSync(join(tmpdir(), "deskmate-aws-rollback-down-"));
  const single = oneServiceConfig();
  const oldBody = JSON.stringify({ contract: 1, tools: [], skills: [{ path: "skills/old/SKILL.md", content: "old" }] });
...
test("rollback refuses an incomplete manifest before mutating ECS", async () => {
  const dir = mkdtempSync(join(tmpdir(), "deskmate-aws-rollback-incomplete-"));
  const multi = twoServiceConfig();
  const fake = statefulAws(
    dir,
    multi,
...
test("secrets push never creates secret containers outside Terraform", async () => {
  const dir = mkdtempSync(join(tmpdir(), "deskmate-aws-push-"));
  const secretsConfig: DeskmateConfig = { ...oneServiceConfig(), env: {} };
  const operator = computedSecrets(secretsConfig).filter(
    (secret) => secret.managedBy === "operator" && secret.required,
  );
...
test("aws logs never reinterprets --tail and interleaves all workloads instead of blocking", async () => {
  const dir = mkdtempSync(join(tmpdir(), "deskmate-aws-logs-"));
...
test("AWS virtual Slack logs resolve to the core task", () => {
  const dir = mkdtempSync(join(tmpdir(), "deskmate-aws-slack-logs-"));
  const fake = fakeAws(dir, `console.log("");`);
  try {
...
test("AWS status batches DescribeServices at the API limit and surfaces failures", () => {
  const dir = mkdtempSync(join(tmpdir(), "deskmate-aws-status-batches-"));
  const services = Object.fromEntries(
...
test("AWS down holds the deploy lease across the ECS mutation", async () => {
  const dir = mkdtempSync(join(tmpdir(), "deskmate-aws-down-lease-"));
  const fake = statefulAws(dir, oneServiceConfig());
  try {
...
test("AWS lease acquisition surfaces a held lease and skips release", async () => {
  const dir = mkdtempSync(join(tmpdir(), "deskmate-aws-lease-held-"));
  const fake = fakeAws(
    dir,
    `
...
console.log("");`,
  ...
  try {
    await assert.rejects(
      ...
      /another Deskmate operation holds the "deploy" lease in acme-deskmate-deploy-locks/,
    );
...
test("AWS live check rejects downed services and classifies probe errors as live drift", async () => {
  const dir = mkdtempSync(join(tmpdir(), "deskmate-aws-live-runtime-"));
  const single = oneServiceConfig();
...
test("AWS live check with an unresolved sandbox pin still evaluates core runtime health", async () => {
  const dir = mkdtempSync(join(tmpdir(), "deskmate-aws-live-unresolved-pin-"));
  const base = oneServiceConfig();
  const single: DeskmateConfig = {
    ...base,
...
test("AWS live check rejects an ingress target group without a healthy target", async () => {
  const dir = mkdtempSync(join(tmpdir(), "deskmate-aws-live-health-"));
  const single = oneServiceConfig();
  const taskArn = "arn:aws:ecs:us-west-2:123456789012:task-definition/acme-core:1";
  const fake = statefulAws(dir, single, manifestItems([{ id: "current", tasks: { core: taskArn } }], "current"));
...
test("AWS live check rejects a reachable public URL returning a server error", async () => {
  const dir = mkdtempSync(join(tmpdir(), "deskmate-aws-live-http-"));
  const single = oneServiceConfig();
  const taskArn = "arn:aws:ecs:us-west-2:123456789012:task-definition/acme-core:1";
  const fake = statefulAws(dir, single, manifestItems([{ id: "current", tasks: { core: taskArn } }], "current"));
...
test("AWS live check uses the package-pinned source image without consulting mutable tags", async () => {
  const dir = mkdtempSync(join(tmpdir(), "deskmate-aws-live-manifest-"));
  const single = oneServiceConfig();
  const taskArn = "arn:aws:ecs:us-west-2:123456789012:task-definition/acme-core:1";
  const fake = statefulAws(
    dir,
  ...
  state.definitions[taskArn] = {
    ...renderTaskDefinition(
      single,
      "core",
      `123456789012.dkr.ecr.us-west-2.amazonaws.com/deskmate-core@sha256:${"a".repeat(64)}`,
      arns,
    ),
  ...
  try {
    ...
    const otherLabel: DeskmateConfig = { ...single, aws: { ...single.aws!, imageLabel: "other-release" } };
    await assert.rejects(
...
test("AWS live check detects prebuilt plugin image drift from current config", async () => {
  const dir = mkdtempSync(join(tmpdir(), "deskmate-aws-live-plugin-image-"));
  const single = oneServiceConfig();
  const pluginConfig: DeskmateConfig = {
    ...single,
    plugins: [{ name: "linear", image: "ghcr.io/acme/linear:1" }],
      ...
      [
        {
          ...
          tasks,
          imageProvenance: {
            core: { kind: "configured", source: "ghcr.io/deskmate/deskmate-core:0.1.0" },
  ...
  state.definitions[tasks.linear] = {
    ...renderTaskDefinition(
      pluginConfig,
      "linear",
      `123456789012.dkr.ecr.us-west-2.amazonaws.com/deskmate-linear@sha256:${"a".repeat(64)}`,
      arns,
    ),
  ...
  try {
    await assert.rejects(
      ...
      new RegExp(`linear: image drift .*desired .*deskmate-linear@sha256:${"b".repeat(64)}`),
    );
  ...
  await assert.rejects(
    () =>
      awsUp(oneServiceConfig(), process.cwd(), {
        dryRun: true,
        buildFrom: true,
        buildFromPath: "/definitely/not/a/deskmate/checkout",
      }),
    /not a Deskmate checkout/,
  );
});

test("AWS mutations release the deploy lease when their initial service snapshot fails", async () => {
  const dir = mkdtempSync(join(tmpdir(), "deskmate-aws-snapshot-lease-"));
  const single = oneServiceConfig();
...
test("AWS service rollback includes an update whose successful response was lost", async () => {
  const dir = mkdtempSync(join(tmpdir(), "deskmate-aws-update-response-"));
  const single = oneServiceConfig();
...
test("AWS down refuses foreign exact-name services before acquiring the lease or mutating ECS", async () => {
  const dir = mkdtempSync(join(tmpdir(), "deskmate-aws-foreign-service-"));
  const single = oneServiceConfig();
...
test("AWS up refuses foreign exact-name services before acquiring the lease or publishing images", async () => {
  const dir = mkdtempSync(join(tmpdir(), "deskmate-aws-foreign-service-up-"));
  const single = oneServiceConfig();
  const fake = statefulAws(dir, single, {}, { foreignServiceTags: true });
  try {
...
test("AWS plan uses the package-pinned source image without consulting or mutating mutable labels", async () => {
  const dir = mkdtempSync(join(tmpdir(), "deskmate-aws-plan-source-"));
  const single = oneServiceConfig();
  const taskArn = "arn:aws:ecs:us-west-2:123456789012:task-definition/acme-core:1";
  const fake = statefulAws(
    dir,
  ...
  state.definitions[taskArn] = {
    ...renderTaskDefinition(
      single,
      "core",
      `123456789012.dkr.ecr.us-west-2.amazonaws.com/deskmate-core@sha256:${"b".repeat(64)}`,
      arns,
    ),
    taskDefinitionArn: taskArn,
  };
  ...
  try {
    ...
    assert.doesNotMatch(readFileSync(dockerLog, "utf8"), /buildx imagetools inspect/);
    assert.match(lines.join("\n"), new RegExp(`deskmate-core@sha256:${"a".repeat(64)}`));
  } finally {
    console.log = log;
    process.env.PATH = priorPath;
    fake.restore();
...
test("aws up resolves image digests only while holding the deploy lease", async () => {
  const dir = mkdtempSync(join(tmpdir(), "deskmate-aws-up-"));
  const dockerLog = join(dir, "docker.log");
  writeFileSync(dockerLog, "");
  const dockerBin = join(dir, "docker");
  writeFileSync(
  ...
  try {
    ...
    assert.match(
      calls,
      /ecs register-task-definition .*--tags \[\{"key":"ManagedBy","value":"deskmate-cli"\},\{"key":"Deployment","value":"acme"\}\]/,
    );
    const lease = calls.indexOf("dynamodb put-item");
    ...
    assert.match(calls, /ecr batch-delete-image .*imageTag=deskmate-staging/);
    assert.ok(readFileSync(dockerLog, "utf8").includes("imagetools create"), "the image was pushed via docker");
    assert.match(readFileSync(dockerLog, "utf8"), /--tag [^\s]+\/deskmate-core:deskmate-staging/);
  } finally {
    console.log = log;
    process.env.PATH = priorPath;
    fake.restore();
...
test("AWS up requires a complete trusted baseline before a partial deployment", async () => {
  const dir = mkdtempSync(join(tmpdir(), "deskmate-aws-partial-up-"));
  const multi = twoServiceConfig();
      ...
      [
        {
          ...
          imageProvenance: {
            ...
            "web-ui": { kind: "configured", source: "ghcr.io/deskmate/deskmate-web-ui:0.1.0" },
          },
        },
      ],
      "baseline",
  ...
  state.definitions[tasks["web-ui"]] = {
    containerDefinitions: [
      {
        name: "web-ui",
        image: `123456789012.dkr.ecr.us-west-2.amazonaws.com/deskmate-web-ui@sha256:${"d".repeat(64)}`,
      },
    ],
  };
...
test("AWS up cleans staging tags when ECS deployment fails", async () => {
  const dir = mkdtempSync(join(tmpdir(), "deskmate-aws-up-cleanup-"));
  const dockerBin = join(dir, "docker");
  writeFileSync(dockerBin, "#!/bin/sh\nexit 0\n");
  chmodSync(dockerBin, 0o755);
...
test("AWS up succeeds while a protected old task keeps the rollout from completing, even with a historical failed task", async () => {
  const dir = mkdtempSync(join(tmpdir(), "deskmate-aws-up-drain-"));
  const dockerBin = join(dir, "docker");
  writeFileSync(dockerBin, "#!/bin/sh\nexit 0\n");
  chmodSync(dockerBin, 0o755);
  const drainConfig = (): DeskmateConfig => {
    const base = oneServiceConfig();
...
test("AWS up tolerates a transient describe-services failure while polling the rollout", async () => {
  const dir = mkdtempSync(join(tmpdir(), "deskmate-aws-up-transient-describe-"));
  const dockerBin = join(dir, "docker");
  writeFileSync(dockerBin, "#!/bin/sh\nexit 0\n");
  chmodSync(dockerBin, 0o755);
...
test("AWS up aborts failed tasks only after four polls with no replacement running — longer than any single ENI/pull flake", async () => {
  const dir = mkdtempSync(join(tmpdir(), "deskmate-aws-up-failed-tasks-"));
  const dockerBin = join(dir, "docker");
  writeFileSync(dockerBin, "#!/bin/sh\nexit 0\n");
  chmodSync(dockerBin, 0o755);
...
test("AWS up survives a three-poll failed-task flake that ECS replaces — the window a transient ENI/pull failure needs", async () => {
  const dir = mkdtempSync(join(tmpdir(), "deskmate-aws-up-transient-failed-task-"));
  const dockerBin = join(dir, "docker");
  writeFileSync(dockerBin, "#!/bin/sh\nexit 0\n");
  chmodSync(dockerBin, 0o755);
...
test("AWS up survives alternating single-service stale reads — only the same workload's same failure on consecutive polls confirms an abort", async () => {
  const dir = mkdtempSync(join(tmpdir(), "deskmate-aws-up-alternating-stale-"));
  const dockerBin = join(dir, "docker");
  writeFileSync(dockerBin, `#!/usr/bin/env node\nconsole.log("Digest: sha256:${"a".repeat(64)}");\n`);
  chmodSync(dockerBin, 0o755);
  const multi = twoServiceConfig();
...
test("AWS up still fails fast on a FAILED rollout state — the ECS circuit-breaker verdict needs no flake window", async () => {
  const dir = mkdtempSync(join(tmpdir(), "deskmate-aws-up-rollout-failed-"));
  const dockerBin = join(dir, "docker");
  writeFileSync(dockerBin, "#!/bin/sh\nexit 0\n");
  chmodSync(dockerBin, 0o755);
...
test("AWS up preserves the staging tag when stable-label promotion fails", async () => {
  const dir = mkdtempSync(join(tmpdir(), "deskmate-aws-up-promotion-"));
  const dockerBin = join(dir, "docker");
  writeFileSync(dockerBin, "#!/bin/sh\nexit 0\n");
  chmodSync(dockerBin, 0o755);
  ...
  try {
    ...
    assert.doesNotMatch(calls, /ecr batch-delete-image .*imageTag=deskmate-/);
  } finally {
    process.env.PATH = priorPath;
    fake.restore();
    rmSync(dir, { recursive: true, force: true });
...
test("AWS up treats an already-current stable label as successful promotion", async () => {
  const dir = mkdtempSync(join(tmpdir(), "deskmate-aws-up-current-label-"));
  const dockerBin = join(dir, "docker");
  writeFileSync(dockerBin, "#!/bin/sh\nexit 0\n");
  chmodSync(dockerBin, 0o755);
...
test("AWS up keeps a healthy rollout and its staging tag when the manifest write fails", async () => {
  const dir = mkdtempSync(join(tmpdir(), "deskmate-aws-up-manifest-failure-"));
  const dockerBin = join(dir, "docker");
  writeFileSync(dockerBin, "#!/bin/sh\nexit 0\n");
  chmodSync(dockerBin, 0o755);
  ...
  try {
    ...
    const calls = readFileSync(fake.log, "utf8");
    assert.doesNotMatch(calls, /ecr batch-delete-image .*imageTag=deskmate-staging/);
...
test("sandbox pin repoints a live core through a re-rendered task and records the manifest under the deployed baseline's label", async () => {
  const dir = mkdtempSync(join(tmpdir(), "deskmate-aws-pin-live-"));
  const single = flySandboxPinlessConfig();
  const fake = statefulAws(
    dir,
    single,
...
test("sandbox pin on a scaled-to-zero core records a carried manifest without touching ECS", async () => {
  const dir = mkdtempSync(join(tmpdir(), "deskmate-aws-pin-carried-"));
  const single = flySandboxPinlessConfig();
  const fake = statefulAws(
    dir,
    single,
...
test("a failed sandbox-pin rollout restores the prior core task and records no manifest", async () => {
  const dir = mkdtempSync(join(tmpdir(), "deskmate-aws-pin-rollback-"));
  const single = flySandboxPinlessConfig();
  const fake = statefulAws(
    dir,
    single,
...
test("a manifest-write failure after the pin repoint is recoverable: the retry records it and up keeps the published pin", async () => {
  const dir = mkdtempSync(join(tmpdir(), "deskmate-aws-pin-manifest-failure-"));
  const dockerBin = join(dir, "docker");
  writeFileSync(dockerBin, `#!/usr/bin/env node\nconsole.log("Digest: sha256:${"a".repeat(64)}");\n`);
  chmodSync(dockerBin, 0o755);
  const single = flySandboxPinlessConfig();
  ...
  process.env.PATH = `${dir}:${priorPath}`;
  try {
    await assert.rejects(() => awsPinSandbox(single, NEW_PIN), /rerun `deskmate sandbox publish` to record the pin/);
...
test("a manifest-write failure rerun with a different digest still converges: publish repoints to the rebuilt pin and up keeps it", async () => {
  const dir = mkdtempSync(join(tmpdir(), "deskmate-aws-pin-rerun-digest-"));
  const dockerBin = join(dir, "docker");
  writeFileSync(dockerBin, `#!/usr/bin/env node\nconsole.log("Digest: sha256:${"a".repeat(64)}");\n`);
  chmodSync(dockerBin, 0o755);
  const single = flySandboxPinlessConfig();
  ...
  process.env.PATH = `${dir}:${priorPath}`;
  try {
    await assert.rejects(() => awsPinSandbox(single, FAILED_PIN), /rerun `deskmate sandbox publish` to record the pin/);
...
test("sandbox publish refuses to record a core manually repointed at a different workload image", async () => {
  const dir = mkdtempSync(join(tmpdir(), "deskmate-aws-pin-manual-repoint-"));
  const single = flySandboxPinlessConfig();
  const fake = statefulAws(
    dir,
    single,
  ...
  try {
    ...
    definition.containerDefinitions[0]!.image = `123456789012.dkr.ecr.us-west-2.amazonaws.com/deskmate-core@sha256:${"9".repeat(64)}`;
    ...
    await assert.rejects(
      ...
      /differs from the deployment manifest's core task beyond the sandbox pin; run `deskmate up --yes` to converge first/,
    );
    const after = JSON.parse(readFileSync(fake.state, "utf8"));
    assert.equal(
...
test("up resolves the sandbox pin from the deployment manifest once the config override is removed", async () => {
  const dir = mkdtempSync(join(tmpdir(), "deskmate-aws-pin-from-manifest-"));
  const dockerBin = join(dir, "docker");
  writeFileSync(dockerBin, `#!/usr/bin/env node\nconsole.log("Digest: sha256:${"a".repeat(64)}");\n`);
  chmodSync(dockerBin, 0o755);
  const single = flySandboxPinlessConfig();
...
test("up on a fly-sandbox stack fails closed before mutation when no pin source or no sandbox.app exists", async () => {
  const single = flySandboxPinlessConfig();
  const bareDir = mkdtempSync(join(tmpdir(), "deskmate-aws-pin-missing-"));
  ...
  const noApp: DeskmateConfig = { ...single, sandbox: { backend: "sprites" } };
  const noAppDir = mkdtempSync(join(tmpdir(), "deskmate-aws-pin-no-app-"));
  const fake = statefulAws(
    noAppDir,
    noApp,
    manifestItems(
...
test("an --only deploy that skips core needs no pin source and records the pin baked into the carried core task", async () => {
  ...
  const overrideDir = mkdtempSync(join(tmpdir(), "deskmate-aws-only-pin-carry-"));
  ...
  const pinlessDir = mkdtempSync(join(tmpdir(), "deskmate-aws-only-pinless-"));
  const pinlessDocker = join(pinlessDir, "docker");
  writeFileSync(pinlessDocker, "#!/bin/sh\nexit 0\n");
  chmodSync(pinlessDocker, 0o755);
  const pinless: DeskmateConfig = { ...twoServiceConfig(), sandbox: { backend: "sprites", app: "acme-sandboxes" } };
...
test("AWS front door tolerates exactly one extra port-80 HTTPS-redirect listener; any other extra listener still fails", async () => {
  const dir = mkdtempSync(join(tmpdir(), "deskmate-aws-front-door-listeners-"));
  const dockerBin = join(dir, "docker");
  writeFileSync(dockerBin, `#!/usr/bin/env node\nconsole.log("Digest: sha256:${"a".repeat(64)}");\n`);
  chmodSync(dockerBin, 0o755);
  const priorPath = process.env.PATH;
...
test("AWS live check accepts a successful deploy mid-drain: PRIMARY at full strength (historical failed task included) while a protected old task keeps the rollout IN_PROGRESS", async () => {
  const dir = mkdtempSync(join(tmpdir(), "deskmate-aws-check-drain-"));
  const single = oneServiceConfig();
  const taskArn = "arn:aws:ecs:us-west-2:123456789012:task-definition/acme-core:1";
  const fake = statefulAws(
    dir,
...
test("AWS renders config secretEnv extras and aliases as Secrets Manager references", () => {
  ...
  assert.equal(
    ...
    "arn:aws:secretsmanager:us-west-2:123456789012:secret:acme/deskmate/OPENAI_API_KEY",
  );
  assert.equal(
    secrets.SLACK_AUX_BOT_TOKEN,
    "arn:aws:secretsmanager:us-west-2:123456789012:secret:acme/deskmate/SLACK_AUX_BOT_TOKEN",
  ...
  const portalImage = `123456789012.dkr.ecr.us-west-2.amazonaws.com/deskmate-portal@sha256:${"a".repeat(64)}`;
  const portalSecrets = (
