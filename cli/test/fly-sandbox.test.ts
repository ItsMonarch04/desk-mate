...
test("the fly target derives a plugin fly.toml wiring it to the core over 6PN", () => {
  ...
  assert.match(toml, /app = "deskmate-linear"/, "app is <appPrefix>-<name>");
  assert.match(toml, /primary_region = "sjc"/);
  assert.match(toml, /CORE_API_URL = "http:\/\/deskmate-core\.internal:8080"/, "plugin reaches core over 6PN");
...
test("--only rejects a name that is neither a service nor a plugin (before any Fly call)", async () => {
  ...
  const emptyDir = mkdtempSync(join(tmpdir(), "deskmate-fly-only-"));
...
test("the Fly S3 probe is valid CommonJS that reports async failures", () => {
  const dir = mkdtempSync(join(tmpdir(), "deskmate-fly-s3-probe-syntax-"));
  try {
...
test("fly secrets push stages a dual-role secret under BOTH names on the core app", async () => {
  const dir = mkdtempSync(join(tmpdir(), "deskmate-fly-push-"));
  const config: DeskmateConfig = {
    contract: 1,
    orgId: "acme",
    publicUrl: "https://acme.example.com",
...
test("fly secrets push warns that staged secrets are not live when machines are running", async () => {
  const dir = mkdtempSync(join(tmpdir(), "deskmate-fly-push-staged-warn-"));
  const config: DeskmateConfig = {
    contract: 1,
    orgId: "acme",
    publicUrl: "https://acme.example.com",
...
`,
  ...
  try {
    ...
    assert.ok(warnings.some((line) => line.includes("run `deskmate up`")));
  } finally {
    console.log = log;
...
test("fly secrets push stays quiet about staging when no machines are running", async () => {
  const dir = mkdtempSync(join(tmpdir(), "deskmate-fly-push-staged-quiet-"));
  const config: DeskmateConfig = {
    contract: 1,
    orgId: "acme",
    publicUrl: "https://acme.example.com",
...
test("fly secrets push removes the disabled Fly app publisher token", async () => {
  const dir = mkdtempSync(join(tmpdir(), "deskmate-fly-push-publisher-off-"));
  const config: DeskmateConfig = {
    contract: 1,
    orgId: "acme",
    publicUrl: "https://acme.example.com",
...
test("fly secrets push falls back to an ambient secret when the scaffold entry is blank", async () => {
  const dir = mkdtempSync(join(tmpdir(), "deskmate-fly-push-blank-"));
  const config: DeskmateConfig = {
    contract: 1,
    orgId: "acme",
    publicUrl: "https://acme.example.com",
...
test("fly live check requires deployed machines and a healthy public endpoint", async () => {
  const dir = mkdtempSync(join(tmpdir(), "deskmate-fly-live-"));
  const config: DeskmateConfig = {
    contract: 1,
    orgId: "acme",
    publicUrl: "https://deskmate.example.test",
    target: "fly",
    region: "sjc",
    flyOrg: "personal",
...
if (a.startsWith("apps list")) console.log(JSON.stringify(["core", "web-ui", "portal"].map((service) => ({ Name: "acme-" + service }))));
  ...
  try {
    await flyCheckLive(config, dir, {
      fetchImpl: async (url) => {
        assert.equal(url, "https://deskmate.example.test/healthz");
...
test("fly live readiness rejects the wrong organization identity, region, and rendered env", async () => {
  const dir = mkdtempSync(join(tmpdir(), "deskmate-fly-live-identity-"));
  const config: DeskmateConfig = {
    contract: 1,
    orgId: "acme",
    publicUrl: "https://deskmate.example.test",
...
if (a.startsWith("apps list")) console.log(JSON.stringify([{ Name: "acme-core" }]));
  ...
  config: { image: "registry.fly.io/app@sha256:abc", env: { DESKMATE_DEPLOYMENT_ID: "deskmate-v2:other-org:acme:acme" } }
}] }));
else if (a.startsWith("checks list")) console.log(JSON.stringify({ machine: [{ status: "passing" }] }));
else console.log("ok");`,
  );
  try {
    await assert.rejects(
      ...
      (error: unknown) => {
        assert.match((error as Error).message, /lack deployment identity deskmate-v2:personal:acme:acme/);
...
test("fly live readiness rejects the wrong region after deployment identity matches", async () => {
  const dir = mkdtempSync(join(tmpdir(), "deskmate-fly-live-region-"));
  const config: DeskmateConfig = {
    contract: 1,
    orgId: "acme",
    publicUrl: "https://deskmate.example.test",
...
test("fly live readiness rejects rendered environment drift after identity and region match", async () => {
  const dir = mkdtempSync(join(tmpdir(), "deskmate-fly-live-env-"));
  const config: DeskmateConfig = {
    contract: 1,
    orgId: "acme",
    publicUrl: "https://deskmate.example.test",
...
test("fly live readiness requires the generated TCP check for plugins", async () => {
  const dir = mkdtempSync(join(tmpdir(), "deskmate-fly-live-plugin-"));
  const config: DeskmateConfig = {
    contract: 1,
    orgId: "acme",
    publicUrl: "https://deskmate.example.test",
...
test("fly live readiness rejects a plugin whose TCP check is not passing", async () => {
  const dir = mkdtempSync(join(tmpdir(), "deskmate-fly-live-plugin-failed-"));
  const config: DeskmateConfig = {
    contract: 1,
    orgId: "acme",
    publicUrl: "https://deskmate.example.test",
...
test("fly live readiness fails when core cannot round-trip durable object storage", async () => {
  const dir = mkdtempSync(join(tmpdir(), "deskmate-fly-live-storage-"));
  const config: DeskmateConfig = {
    contract: 1,
    orgId: "acme",
    publicUrl: "https://deskmate.example.test",
...
test("fly live readiness rejects a healthy machine on the wrong configured image digest", async () => {
  const dir = mkdtempSync(join(tmpdir(), "deskmate-fly-live-image-"));
...
test("fly live readiness resolves deployment tags to their immutable image digest", async () => {
  const dir = mkdtempSync(join(tmpdir(), "deskmate-fly-live-image-tag-"));
...
test("fly secrets push rejects weak signing keys before staging anything", async () => {
  const dir = mkdtempSync(join(tmpdir(), "deskmate-fly-weak-secret-"));
  const config: DeskmateConfig = {
    contract: 1,
    orgId: "acme",
    publicUrl: "https://acme.example.com",
...
test("fly secrets push refuses an unmarked pre-existing app", async () => {
  const dir = mkdtempSync(join(tmpdir(), "deskmate-fly-owner-"));
  const config: DeskmateConfig = {
    contract: 1,
    orgId: "acme",
    publicUrl: "https://acme.example.com",
...
test("fly secrets push refuses a same-named app outside the configured Fly organization", async () => {
  const dir = mkdtempSync(join(tmpdir(), "deskmate-fly-wrong-org-"));
  const config: DeskmateConfig = {
    contract: 1,
    orgId: "acme",
    publicUrl: "https://acme.example.com",
...
test("fly rollback joins a bare digest with @ and records the pin in the config file", () => {
  const dir = mkdtempSync(join(tmpdir(), "deskmate-fly-rollback-"));
  const appPrefix = "deskmaterbtest";
  ...
  const configPath = join(dir, "deskmate.config.jsonc");
  writeFileSync(
    configPath,
    `${JSON.stringify({ contract: 1, orgId: "acme", sandbox: { app: `${appPrefix}-sb` } }, null, 2)}\n`,
  );
  const marker = `DESKMATE_OWNER_${createHash("sha256").update(`deskmate-v2:personal:acme:${appPrefix}`).digest("hex").slice(0, 16).toUpperCase()}`;
  const fake = fakeFly(
    dir,
    `
...
test("fly rollback validates an image pin before changing the committed config", () => {
  const dir = mkdtempSync(join(tmpdir(), "deskmate-fly-rollback-invalid-"));
  const configPath = join(dir, "deskmate.config.jsonc");
...
test("fly rollback refuses to change the committed config when the image cannot be resolved", () => {
  const dir = mkdtempSync(join(tmpdir(), "deskmate-fly-rollback-missing-"));
  const configPath = join(dir, "deskmate.config.jsonc");
  const raw = JSON.stringify({ contract: 1, orgId: "acme", sandbox: { app: "acme-sb" } }, null, 2) + "\n";
  writeFileSync(configPath, raw);
  const docker = join(dir, "docker");
...
test("fly sandbox pin re-derives the core [env] and unsets any stale FLY_BASE_IMAGE secret", () => {
  const dir = mkdtempSync(join(tmpdir(), "deskmate-fly-pin-"));
  const appPrefix = "deskmatepintest";
...
test("fly sandbox pin refuses to mutate an unmarked app with the configured name", () => {
  const dir = mkdtempSync(join(tmpdir(), "deskmate-fly-pin-unowned-"));
  const appPrefix = "deskmatepinunowned";
...
if (a.startsWith("apps list")) console.log(JSON.stringify([{ Name: "${appPrefix}-core" }]));
  ...
  try {
    assert.throws(
      ...
      /not marked as owned by deployment deskmate-v2:personal:acme:deskmatepinunowned/,
    );
    const calls = readFileSync(fake.log, "utf8");
...
test("fly secrets push stages a secretEnv alias under its declared env name on its service's app", async () => {
  const dir = mkdtempSync(join(tmpdir(), "deskmate-fly-alias-"));
  const config: DeskmateConfig = {
    contract: 1,
    orgId: "acme",
    publicUrl: "https://acme.example.com",
