...
function makeDeployment(config: Record<string, unknown>, setup: (dir: string) => void = () => {}): string {
  const dir = mkdtempSync(join(tmpdir(), "deskmate-wiring-"));
  writeFileSync(
...
test("--sandbox-dir sources the layer from a shared dir while config stays in the deployment dir", async () => {
  const dir = makeDeployment({});
  const shared = mkdtempSync(join(tmpdir(), "deskmate-shared-"));
  sandboxLayer(shared);
  try {
...
test("model → AGENT_MODEL and host ports follow the offset map (core+0, portal+1, web-ui+2, admin+3)", async () => {
  const dir = makeDeployment({ model: "claude-opus-4-8", services: ["core", "portal", "web-ui", "admin"] });
  try {
    const out = await plan(dir);
    assert.match(out, /AGENT_MODEL/);
...
test("DESKMATE_BASE_PORT overrides the host port base for one run", async () => {
  const dir = makeDeployment({ services: ["core"] });
  const prev = process.env.DESKMATE_BASE_PORT;
  process.env.DESKMATE_BASE_PORT = "9000";
  try {
    const out = await plan(dir);
    assert.match(out, /host :9000/);
  } finally {
    if (prev === undefined) delete process.env.DESKMATE_BASE_PORT;
    else process.env.DESKMATE_BASE_PORT = prev;
    rmSync(dir, { recursive: true, force: true });
  }
});
