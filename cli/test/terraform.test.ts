...
const DEPLOY_IMAGE = "acme-deskmate-sandbox";
...
test("GCP tfvars contain only deployment-derived coordinates", () => {
  ...
  assert.equal(
    rendered,
    'project_id        = "acme-project"\nregion            = "us-central1"\norg_id            = "acme"\nartifact_registry = "acme-deskmate"\nsecrets_prefix    = "acme-deskmate-"\n',
  );
});
...
test("terraform derives account-scoped infrastructure coordinates", () => {
  ...
  assert.match(awsObjectStoreBucket(config), /^deskmate-acme-[0-9a-f]{12}$/);
  assert.match(
    mainTf,
...
test("GitHub environments remain compatible with AWS scaffolds created before the variable existed", () => {
  ...
  const legacyConfig: DeskmateConfig = {
    ...config,
...
test("AWS module provisions durable encrypted object storage and configurable safe teardown", () => {
  ...
  assert.match(
    mainTf,
    /id\s*= "deskmate-transfer-expiry"[\s\S]*?prefix\s*= var\.transfer_lifecycle_prefix[\s\S]*?days\s*= 1/,
  );
...
test("drift check flags wrong derived values but never operator formatting", () => {
  ...
  const wrongCluster = rendered.replace('"acme-deskmate"', '"other-cluster"');
