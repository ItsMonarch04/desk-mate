...
function sandboxDir(setup: (sb: string) => void): string {
  const dir = mkdtempSync(join(tmpdir(), "deskmate-sbx-build-"));
...
function dryRun(opts: Omit<SandboxBuildOpts, "dryRun" | "config"> & { config?: DeskmateConfig }): string {
  const lines: string[] = [];
...
test("generates a Dockerfile that COPYs each tool executable onto PATH + bakes the presence check", () => {
  ...
  try {
    ...
    assert.match(out, /FROM registry\.invalid\/deskmate\/deskmate-sandbox-base@sha256:a{64}/);
