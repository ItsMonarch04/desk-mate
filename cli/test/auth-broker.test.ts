...
import { CONFIG_FILENAME, loadConfigAt, validatePortalTrust, type DeskmateConfig } from "../src/config.ts";
...
const brokerStack = join(repoRoot, "deploy", "stacks", "broker", "deskmate.config.jsonc");
const brokerConfig = (): DeskmateConfig => loadConfigAt(brokerStack).config;

function configWith(body: string): DeskmateConfig {
  const dir = mkdtempSync(join(tmpdir(), "deskmate-auth-cfg-"));
  try {
...
test("fly derives the portal's whole OIDC block from the broker, over the private network", () => {
  ...
  assert.match(portal, /OIDC_TOKEN_ENDPOINT = "http:\/\/deskmate-auth\.flycast\/token"/);
  assert.match(portal, /OIDC_USERINFO_ENDPOINT = "http:\/\/deskmate-auth\.flycast\/userinfo"/);
  assert.match(portal, /OIDC_JWKS_URI = "http:\/\/deskmate-auth\.flycast\/\.well-known\/jwks\.json"/);
  assert.match(portal, /AUTH_BROKER_UPSTREAM = "http:\/\/deskmate-auth\.flycast"/);
...
test("the external-IdP stack keeps its own OIDC endpoints and gains no broker wiring", () => {
  const acme = loadConfigAt(join(repoRoot, "deploy", "stacks", "acme", "deskmate.config.jsonc")).config;
...
test("docker and AWS wire the broker with parity", () => {
  ...
  const aws = configWith(`{
    ...
    "aws": {
      "accountId": "123456789012", "region": "us-west-2", "cluster": "acme-deskmate",
      "deployRoleArn": "arn:aws:iam::123456789012:role/acme-deskmate-github-deploy",
      "secretsPrefix": "acme/deskmate/", "imageLabel": "latest",
      "networking": { "cloudMapNamespace": "acme.internal" },
      "services": {
        "core": { "ecrRepository": "acme-deskmate-core", "ecsService": "acme-deskmate-core", "cpu": 2048, "memory": 4096 },
        "web-ui": { "ecrRepository": "acme-deskmate-web-ui", "ecsService": "acme-deskmate-web-ui", "cpu": 512, "memory": 1024 },
        "admin": { "ecrRepository": "acme-deskmate-admin", "ecsService": "acme-deskmate-admin", "cpu": 512, "memory": 1024 },
        "portal": { "ecrRepository": "acme-deskmate-portal", "ecsService": "acme-deskmate-portal", "cpu": 512, "memory": 1024 },
        "auth": { "ecrRepository": "acme-deskmate-auth", "ecsService": "acme-deskmate-auth", "cpu": 256, "memory": 512 }
      }
    }
  }`);
