...
if (args[0] === "apps" && args[1] === "create") {
  ...
  console.log('DESKMATE_LAYER_RESPONSE=' + JSON.stringify({ status: 200, body: JSON.stringify({ version: 1, contentHash: "0123456789abcdef" }) }));
} else {
...
test("fly up emits phase timings and appends a GitHub step summary", () => {
  const dir = mkdtempSync(join(tmpdir(), "deskmate-fly-timing-"));
  ...
  const stdout = execFileSync(
    ...
    [
      ...
      "up",
      "--config",
      join(repoRoot, "deploy", "stacks", "acme", "deskmate.config.jsonc"),
      "--only",
      "core",
      "--image-from",
      "source",
  ...
  assert.match(stdout, /timing deskmate\/core: app ensure \d+(?:ms|\.\d+s)/);
  assert.match(stdout, /timing deskmate\/core: secret checks \d+(?:ms|\.\d+s)/);
  assert.match(stdout, /timing deskmate\/core: Postgres ensure \d+(?:ms|\.\d+s)/);
  assert.match(stdout, /timing deskmate\/core: current image lookup \d+(?:ms|\.\d+s)/);
  assert.match(stdout, /timing deskmate\/core: fly deploy \d+(?:ms|\.\d+s)/);

  const summary = readFileSync(summaryPath, "utf8");
  assert.match(summary, /### Fly deploy timings \(deskmate\)/);
  assert.match(summary, /\| Stack \| Service \| Phase \| Duration \|/);
  assert.match(summary, /\| deskmate \| core \| app ensure \| \d+(?:ms|\.\d+s) \|/);
  assert.match(summary, /\| deskmate \| core \| secret checks \| \d+(?:ms|\.\d+s) \|/);
  assert.match(summary, /\| deskmate \| core \| Postgres ensure \| \d+(?:ms|\.\d+s) \|/);
  assert.match(summary, /\| deskmate \| core \| current image lookup \| \d+(?:ms|\.\d+s) \|/);
  assert.match(summary, /\| deskmate \| core \| fly deploy \| \d+(?:ms|\.\d+s) \|/);
});

test("fly up can deploy a tagged image without consulting the source stack's running image", () => {
  const dir = mkdtempSync(join(tmpdir(), "deskmate-fly-tagged-"));
  ...
  assert.match(stdout, /--image registry\.fly\.io\/deskmate-core:sha123/);
  ...
  assert.equal(
    commands.some(
      (args) =>
        args[0] === "deploy" && args.includes("--image") && args.includes("registry.fly.io/deskmate-core:sha123"),
    ),
    true,
  );
});
...
test("fly image-from fails closed when Fly cannot resolve the running tag to a digest", () => {
  const dir = mkdtempSync(join(tmpdir(), "deskmate-fly-no-digest-"));
...
test("fly image-from resolves by machine id during a mixed rollout", () => {
  const dir = mkdtempSync(join(tmpdir(), "deskmate-fly-mixed-images-"));
  const logPath = join(dir, "fly.log");
  execFileSync(
    process.execPath,
    [
...
test("a fresh Fly deploy stages the direct Managed Postgres URL without attaching the pooled URL", () => {
  const dir = mkdtempSync(join(tmpdir(), "deskmate-fly-fresh-pg-"));
  const logPath = join(dir, "fly.log");
  execFileSync(
    process.execPath,
    [
...
test("Fly preserves an existing DATABASE_URL even when a same-name Managed Postgres cluster exists", () => {
  const dir = mkdtempSync(join(tmpdir(), "deskmate-fly-existing-db-"));
  const logPath = join(dir, "fly.log");
  execFileSync(
    process.execPath,
    [
...
test("Fly redacts credential-bearing Managed Postgres status failures", () => {
  const dir = mkdtempSync(join(tmpdir(), "deskmate-fly-pg-error-"));
  const result = spawnSync(
    process.execPath,
    [
      bin,
...
test("fly up build-only pushes a tagged image without checking runtime deploy secrets", () => {
  const dir = mkdtempSync(join(tmpdir(), "deskmate-fly-build-only-"));
  const logPath = join(dir, "fly.log");
  const stdout = execFileSync(
    process.execPath,
    [
  ...
  assert.match(stdout, /image: deskmate-core -> registry\.fly\.io\/deskmate-core:sha123/);
  const commands = readFileSync(logPath, "utf8")
    .trim()
    .split("\n")
    .map((line) => JSON.parse(line) as string[]);
...
test("fly up build-only dry-run plans without pushing an image", () => {
  const dir = mkdtempSync(join(tmpdir(), "deskmate-fly-build-only-dry-run-"));
  const logPath = join(dir, "fly.log");
  const stdout = execFileSync(
    process.execPath,
    [
