...
test("the package uses the organization scope while keeping the deskmate command", () => {
...
test(
  ...
  { timeout: 60_000 },
  async () => {
    const dir = mkdtempSync(join(tmpdir(), "deskmate-package-"));
    ...
    try {
      ...
      registry = undefined;
      const bin = join(deployment, "node_modules", ".bin", "deskmate");
      const awsBin = join(awsDeployment, "node_modules", ".bin", "deskmate");
      rmSync(tarball);

      const deploymentConfig = join(deployment, "deskmate.config.jsonc");
      writeFileSync(
        deploymentConfig,
      ...
      assert.ok(existsSync(join(deployment, ".codex", "skills", "deploy-deskmate", "SKILL.md")));
      for (const provider of ["fly", "aws", "slack"]) {
        assert.ok(existsSync(join(deployment, ".codex", "skills", "deploy-deskmate", "references", `${provider}.md`)));
      }
      assert.equal(statSync(join(deployment, ".env")).mode & 0o777, 0o600);
      assert.doesNotMatch(
        readFileSync(join(deployment, "deployment.md"), "utf8"),
        /DESKMATE_REPO|cli\/bin\/deskmate\.ts|fresh Deskmate clone|source checkout/i,
      );
      assert.match(
        readFileSync(join(deployment, "deployment.md"), "utf8"),
      ...
      assert.match(
        ...
        /deskmate-sandbox-base@sha256:a{64}/,
      );
      ...
      const awsConfigPath = join(awsDeployment, "deskmate.config.jsonc");
      writeFileSync(
        awsConfigPath,
      ...
      const cluster = "acme-aws-deskmate";
...
else if (command === "lambda-microvms get-microvm-image") json({ imageArn: "arn:aws:lambda:us-west-2:000000000000:microvm-image:acme-aws-deskmate-sandbox" });
...
`,
      ...
      assert.match(awsPlan, /000000000000\.dkr\.ecr\.us-west-2\.amazonaws\.com\/acme-aws-deskmate-core@sha256:a{64}/);
      ...
      assert.ok(!packed[0]!.files.some(({ path }) => path === "src/contract.ts" || path === "bin/deskmate.ts"));
    } finally {
