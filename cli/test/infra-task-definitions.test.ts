...
if (text.includes("sts get-caller-identity")) console.log(${JSON.stringify(account)});
  ...
  console.log(JSON.stringify({ tags: [{ key: "ManagedBy", value: "deskmate-cli" }, { key: "Deployment", value: ${JSON.stringify(deployment)} }] }));
} else console.log("{}");
`,
  );
  chmodSync(bin, 0o755);
...
test("infra delete-task-definitions removes every revision of exact configured families in API-sized batches", async () => {
  const dir = mkdtempSync(join(tmpdir(), "deskmate-infra-task-definitions-"));
