...
test("init scaffolds a loadable config, generated local secrets, and a valid sandbox/ layer", () => {
  const base = mkdtempSync(join(tmpdir(), "deskmate-init-"));
  try {
    ...
    assert.equal(config.env.core?.HARNESS, "claude");
    ...
    assert.match(manifest, /^ {2}name: Deskmate$/m);
    assert.match(manifest, /^ {4}display_name: Deskmate$/m);
...
test("init --target fly scaffolds the full hosted topology and both Slack apps", () => {
  const dir = mkdtempSync(join(tmpdir(), "deskmate-init-fly-"));
  try {
...
test("init --email-transport smtp scaffolds smtp keys only and a matching config", () => {
  const dir = mkdtempSync(join(tmpdir(), "deskmate-init-smtp-"));
  try {
...
test("init keeps stable Deskmate Slack branding for long org ids", () => {
  for (const org of ["a".repeat(34), `${"a".repeat(28)}-bcd`]) {
    const dir = mkdtempSync(join(tmpdir(), "deskmate-init-"));
    try {
      quiet(() => runInit({ dir, org }));
      const manifest = readFileSync(join(dir, "slack-app-manifest.yml"), "utf8");
      assert.match(manifest, /^ {2}name: Deskmate$/m);
      assert.match(manifest, /^ {4}display_name: Deskmate$/m);
      assert.ok(manifest.includes(`Deskmate workspace agent for ${org}`));
    } finally {
      rmSync(dir, { recursive: true, force: true });
    }
  }
...
test("init --target aws scaffolds the full hosted topology, Terraform, and the optional Slack bot manifest", () => {
  ...
  try {
    ...
    assert.match(tfvars, /cluster_name\s*= "acme-deskmate"/);
    assert.match(tfvars, /github_repository\s*= "replace-me\/repository"/);
    assert.match(tfvars, /deploy_microvm_image\s*= "acme-deskmate-sandbox"/);
    ...
    for (const command of [
      ...
      "npm exec deskmate -- infra delete-task-definitions --yes",
    ...
    const taskDefinitionCleanup = agents.indexOf("npm exec deskmate -- infra delete-task-definitions --yes");
    assert.ok(
...
test("init --target aws derives stable valid AWS names for a maximum-length org id", () => {
  ...
  const first = mkdtempSync(join(tmpdir(), "deskmate-init-long-"));
  const second = mkdtempSync(join(tmpdir(), "deskmate-init-long-"));
  try {
...
test("init makes .env the final matching gitignore rule before generating signing keys", () => {
  const dir = mkdtempSync(join(tmpdir(), "deskmate-init-existing-ignore-"));
  try {
...
test("init refuses to generate keys into an absent but Git-tracked .env", () => {
  const dir = mkdtempSync(join(tmpdir(), "deskmate-init-tracked-env-"));
  try {
...
test("init refuses a present tracked .env before writing the scaffold", () => {
  const dir = mkdtempSync(join(tmpdir(), "deskmate-init-present-tracked-env-"));
  try {
    execFileSync("git", ["init"], { cwd: dir, stdio: "ignore" });
    writeFileSync(join(dir, ".env"), "previous=value\n");
    execFileSync("git", ["add", "-f", ".env"], { cwd: dir, stdio: "ignore" });
...
test("init merges every AWS secret and generated-state ignore into an existing file", () => {
  const dir = mkdtempSync(join(tmpdir(), "deskmate-init-aws-ignore-"));
  try {
...
test("init preflights package.json and completes an install-first package manifest", () => {
  const invalid = mkdtempSync(join(tmpdir(), "deskmate-init-invalid-package-"));
  const existing = mkdtempSync(join(tmpdir(), "deskmate-init-existing-package-"));
  try {
    ...
    writeFileSync(
      ...
      JSON.stringify({
        dependencies: { "deskmate-cli": "0.1.0", other: "1.0.0" },
      }),
    );
...
test("init preserves an installed local package artifact", () => {
  const dir = mkdtempSync(join(tmpdir(), "deskmate-init-local-package-"));
  try {
    writeFileSync(
...
test("init --target aws vendors a contract-valid Terraform deployment", () => {
  const dir = mkdtempSync(join(tmpdir(), "deskmate-init-aws-"));
  try {
    quiet(() => runInit({ dir, org: "globex", target: "aws" }));
    const { config } = loadConfigInDir(dir);
    assert.equal(config.aws?.cluster, "globex-deskmate");
...
test("the scaffold is an npm-backed deployment repository with no CI coupling and valid Slack YAML", () => {
  ...
  try {
    ...
    for (const provider of ["fly", "aws", "slack", "email"]) {
      assert.ok(existsSync(join(dir, ".codex", "skills", "deploy-deskmate", "references", `${provider}.md`)));
    }
...
test("init --target gcp scaffolds the hosted topology with a gcp block and the Sprites sandbox interim", () => {
  const dir = mkdtempSync(join(tmpdir(), "deskmate-init-gcp-"));
  try {
    ...
    assert.deepEqual(config.secretEnv?.core, { ADMIN_GRANTS: "ADMIN_GRANTS" });
    assert.ok(
      existsSync(join(dir, ".codex", "skills", "deploy-deskmate", "references", "gcp.md")),
    ...
    assert.match(agents, /npm exec deskmate -- secrets push/);
