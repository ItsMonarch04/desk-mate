...
test("check --json reports failures under a stable contract clause id", async () => {
  const dir = mkdtempSync(join(tmpdir(), "deskmate-dispatch-"));
  writeFileSync(
...
test("--env-file that does not exist is an error", async () => {
  ...
  const xdg = mkdtempSync(join(tmpdir(), "deskmate-xdg-"));
...
test("destructive commands reject unknown flags before resolving a deployment", async () => {
  for (const argv of [
    ...
    assert.doesNotMatch(result.out, /no deskmate\.config/);
  }
});
...
test("conformance honors the deploy-wide --config and --sandbox-dir flags", async () => {
  const deployDir = mkdtempSync(join(tmpdir(), "deskmate-dispatch-"));
  const sandboxDir = mkdtempSync(join(tmpdir(), "deskmate-dispatch-sb-"));
  const elsewhere = mkdtempSync(join(tmpdir(), "deskmate-dispatch-cwd-"));
  writeFileSync(
...
test("config get prints raw scalars and JSON objects, honors --target, and fails loudly on unset paths", async () => {
  const dir = mkdtempSync(join(tmpdir(), "deskmate-dispatch-cfg-"));
  writeFileSync(
    join(dir, CONFIG_FILENAME),
    JSON.stringify({
      contract: 1,
  ...
  try {
    ...
    assert.match(usage.out, /usage: deskmate config get/);
  } finally {
    rmSync(dir, { recursive: true, force: true });
  }
});
...
test("--target revalidates the effective provider config", async () => {
  const dir = mkdtempSync(join(tmpdir(), "deskmate-dispatch-target-validation-"));
  writeFileSync(
    join(dir, CONFIG_FILENAME),
    JSON.stringify({
      contract: 1,
...
test("check --json routes failures on structured clause data, not message sniffing", async () => {
  ...
  const sandboxDir = mkdtempSync(join(tmpdir(), "deskmate-dispatch-"));
  ...
  const awsDir = mkdtempSync(join(tmpdir(), "deskmate-dispatch-"));
  writeFileSync(
  ...
  const secretDir = mkdtempSync(join(tmpdir(), "deskmate-dispatch-"));
