...
function writeConfig(extra: Record<string, unknown>): { dir: string; path: string } {
  const dir = mkdtempSync(join(tmpdir(), "deskmate-cfg-"));
  ...
  withConfig(
    {
      ...
      flyOrg: "personal",
      imageFrom: "deskmate",
      deployAppPrefix: "deskmate-d",
    },
    ({ path }) => {
      const { config } = loadConfigAt(path);
      assert.equal(config.appPrefix, "deskmate");
      assert.equal(config.region, "sjc");
      assert.equal(config.flyOrg, "personal");
      assert.equal(config.imageFrom, "deskmate");
      assert.equal(config.deployAppPrefix, "deskmate-d");
    },
  );
});
...
test("AWS config rejects public surfaces without the HTTPS portal and real harnesses over HTTP", () => {
  ...
  const cases: Array<{ label: string; config: Record<string, unknown>; error: RegExp }> = [
    ...
    },
    {
      ...
      error: /HARNESS=claude requires an HTTPS publicUrl/,
    },
    {
  ...
  withConfig(
    {
      ...
      env: {
        core: { HARNESS: "claude" },
        portal: { OIDC_CLIENT_ID: "client", OIDC_ALLOWED_EMAILS: "admin@example.com" },
      },
      aws: aws(["core", "web-ui", "portal"]),
    },
    ({ path }) => {
  ...
  withConfig(
    {
      ...
      publicUrl: "https://agent.acme.example",
      services: ["core", "web-ui", "portal"],
      env: { core: { HARNESS: "claude" }, portal: { OIDC_PRINCIPAL_CLAIM: "email" } },
      aws: aws(["core", "web-ui", "portal"]),
    },
    ({ path }) => {
...
test("a sandbox.app with no pinned image fails closed instead of fabricating a mutable tag", () => {
  withConfig({ sandbox: { app: "acme-sandboxes" } }, ({ path }) => {
    const { config } = loadConfigAt(path);
    assert.throws(() => sandboxCoreEnv(config), /no sandbox layer image is pinned.*deskmate sandbox publish/s);
  });
});
...
test("loadConfigInDir reads deskmate.config.jsonc from the deployment dir (no walk-up)", () => {
  withConfig({}, ({ dir }) => {
    assert.equal(loadConfigInDir(dir).config.orgId, "acme");
  });
  const empty = mkdtempSync(join(tmpdir(), "deskmate-empty-"));
  try {
    assert.throws(() => loadConfigInDir(empty), /no deskmate.config.jsonc/);
  } finally {
...
test("config JSONC accepts comments and trailing commas like tsconfig.json", () => {
  const dir = mkdtempSync(join(tmpdir(), "deskmate-jsonc-"));
  const path = join(dir, CONFIG_FILENAME);
  try {
    writeFileSync(
      path,
...
test("aws.services logGroup and stopTimeout adopt live task-def values and validate their shapes", () => {
  ...
  const services = (core: Record<string, unknown>): Record<string, unknown> => ({
    core: { ecrRepository: "deskmate-core", ecsService: "acme-core", cpu: 2048, memory: 4096, ...core },
  });
  withConfig(
...
test("modelProvider must name a vendor the configured harness can bill", () => {
  withConfig({ modelProvider: "openrouter", env: { core: { HARNESS: "mock" } } }, ({ path }) => {
...
test("env.core.MODEL_PROVIDER is validated as the provider core will actually use", () => {
  ...
  withConfig({ env: { core: { HARNESS: "claude", MODEL_PROVIDER: "bedrock" } } }, ({ path }) => {
...
test("a mock deployment is named as one, and a real harness draws no warning", () => {
  ...
  withConfig({ env: { core: { HARNESS: "claude" } } }, ({ path }) => {
    assert.equal(mockHarnessWarning(loadConfigAt(path).config), undefined);
  });
  withConfig({ target: "fly", appPrefix: "acme", env: { core: {} } }, ({ path }) => {
    assert.equal(mockHarnessWarning(loadConfigAt(path).config), undefined, "the fly template renders HARNESS=claude");
  });
});
