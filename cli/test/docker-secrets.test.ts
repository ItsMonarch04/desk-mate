...
test("docker up delivers secrets via a 0600 env-file, never on the docker argv", { timeout: 60_000 }, async () => {
  const dir = mkdtempSync(join(tmpdir(), "deskmate-docker-secrets-"));
...
test(
  ...
  { timeout: 60_000 },
  async () => {
    const dir = mkdtempSync(join(tmpdir(), "deskmate-docker-secrets-pg-"));
    const xdg = mkdtempSync(join(tmpdir(), "deskmate-docker-secrets-xdg-"));
    const priorPath = process.env.PATH;
    const priorDb = process.env.DATABASE_URL;
    ...
    try {
      ...
      const statePath = join(xdg, "deskmate", "deployments", "sekritpg", "state.json");
      ...
      assert.ok(
        envFiles.includes(`DATABASE_URL=postgres://postgres:${password}@pg:5432/deskmate`),
...
test(
  ...
  { timeout: 60_000 },
  async () => {
    const dir = mkdtempSync(join(tmpdir(), "deskmate-docker-secrets-gate-"));
    const priorPath = process.env.PATH;
    const priorDb = process.env.DATABASE_URL;
...
test(
  ...
  { timeout: 60_000 },
  async () => {
    const dir = mkdtempSync(join(tmpdir(), "deskmate-docker-secrets-nl-"));
    const priorPath = process.env.PATH;
    const priorDb = process.env.DATABASE_URL;
