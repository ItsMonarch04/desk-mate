...
function config(): DeskmateConfig {
  return {
    contract: 1,
    orgId: "acme",
...
if (command === "dynamodb put-item") {
  ...
  console.log(JSON.stringify({Tags:{ManagedBy:"deskmate-cli",Deployment:process.env.FAKE_DEPLOYMENT_TAG || "acme"}}));
...
async function withHarness(env: Record<string, string>, fn: (h: Harness) => Promise<void>): Promise<void> {
  const dir = mkdtempSync(join(tmpdir(), "deskmate-infra-image-"));
...
for (const mode of ["create", "update"] as const) {
  test(`infra build-image ${mode}s the image and records immutable runtime coordinates`, async () => {
    await withHarness({ FAKE_MODE: mode }, async ({ configPath, calls }) => {
      ...
      assert.equal(
        loaded.env.core?.AWS_DEPLOY_EXEC_ROLE_ARN,
        "arn:aws:iam::123456789012:role/acme-deskmate-microvm-exec",
      );
      ...
      assert.match(logged, /--build-role-arn arn:aws:iam::123456789012:role\/acme-deskmate-microvm-build/);
      assert.match(
        logged,
        new RegExp(
      ...
      if (mode === "create") assert.match(logged, /--tags \{"ManagedBy":"deskmate-cli","Deployment":"acme"\}/);
    });
  });
}
  ...
  test(`infra image lifecycle lease serializes build-image against ${contender}`, async () => {
    await withHarness({ FAKE_MODE: "create" }, async ({ configPath, calls }) => {
      ...
      try {
        ...
        await assert.rejects(second, /another Deskmate operation holds the "deploy" lease/);
