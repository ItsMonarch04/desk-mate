...
function deployment(
  ...
  config: Partial<DeskmateConfig> = {},
): { dir: string; config: DeskmateConfig } {
  const dir = mkdtempSync(join(tmpdir(), "deskmate-check-"));
  setup(dir);
...
function check(d: { dir: string; config: DeskmateConfig }): ReturnType<typeof runChecks> {
...
test("every Fly deployment requires durable S3-compatible stores", () => {
  const base = {
    ...
    region: "sjc",
    flyOrg: "personal",
    services: ["core", "portal"] as DeskmateConfig["services"],
  };
