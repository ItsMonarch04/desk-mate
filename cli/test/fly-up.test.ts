...
test("fly up provisions private object storage before enforcing provider secrets", async () => {
  const dir = mkdtempSync(join(tmpdir(), "deskmate-fly-storage-"));
  const fly = join(dir, "fly");
  const log = join(dir, "fly.log");
  const state = join(dir, "storage-created");
  // Derived, not hardcoded, so it tracks the deployment-id format instead of drifting from it.
  const marker = `DESKMATE_OWNER_${createHash("sha256")
    .update("deskmate-v2:personal:acme:acme")
    .digest("hex")
    .slice(0, 16)
    .toUpperCase()}`;
  writeFileSync(
...
`,
  ...
  try {
    ...
    assert.equal(existsSync(join(dir, "deskmate.config.jsonc")), false);
  } finally {
...
test('--only "slack" explains the virtual service runs in-process on the core', async () => {
  const dir = mkdtempSync(join(tmpdir(), "deskmate-fly-up-"));
  const config: DeskmateConfig = {
    contract: 1,
    orgId: "acme2",
