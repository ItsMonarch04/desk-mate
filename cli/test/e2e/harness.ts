...
import { CONFIG_FILENAME, type DeskmateConfig } from "../../src/config.ts";
...
export const bin = join(cliDir, "bin", "deskmate.ts");
...
export function tmp(prefix: string): string {
  return mkdtempSync(join(tmpdir(), `deskmate-e2e-${prefix}-`));
}
...
let gitWorktreeCache: boolean | undefined;
/**
 * The `dev` and `--build-from` paths resolve the checkout through `git rev-parse`, so they
 * cannot run from a source export that has not been initialized as a repository. CI always
 * checks out with git, so these stay enforced there.
 */
export function insideGitWorktree(): boolean {
  if (gitWorktreeCache !== undefined) return gitWorktreeCache;
  try {
    execFileSync("git", ["rev-parse", "--show-toplevel"], { cwd: repoRoot, stdio: "ignore" });
    gitWorktreeCache = true;
  } catch {
    gitWorktreeCache = false;
  }
  return gitWorktreeCache;
}

export const NO_GIT_WORKTREE = "not inside a git worktree";

export function deploymentContainers(orgId: string): string[] {
  try {
    return execFileSync("docker", ["ps", "-a", "--filter", `label=deskmate.org=${orgId}`, "--format", "{{.Names}}"], {
      encoding: "utf8",
    })
      .split("\n")
      .map((s) => s.trim())
...
export const deploymentVolumes = (orgId: string): string[] =>
  existingDockerNames("volume", [`deskmate-${orgId}-pgdata`, `deskmate-${orgId}-coredata`]);
export const deploymentNetworks = (orgId: string): string[] => existingDockerNames("network", [`deskmate-${orgId}`]);
...
export function preexistingServiceImages(services: readonly string[]): string[] {
  try {
    ...
    return services.map((s) => `deskmate-${s}:local`).filter((t) => present.has(t));
  } catch {
    return [];
  }
}
...
export function removeStandInImages(services: readonly string[], orgId: string): void {
  const tags = [...services.map((s) => `deskmate-${s}:local`), `deskmate-${orgId}-widget:local`];
...
if (has("apps", "create")) { console.log("created"); }
  ...
  if (args.includes("--json")) console.log(JSON.stringify({ Machines: [{ config: { image: "registry.fly.io/fake:latest", env: owner ? { DESKMATE_DEPLOYMENT_ID: owner } : {} } }] }));
