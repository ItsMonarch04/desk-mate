# `deskmate` CLI end-to-end tests

These drive the **real** `deskmate` binary as a subprocess and verify each command's
...
- **Isolation.** Every deployment uses a unique org `deskmate-e2e-<label>-<pid>` (so it never collides
  with a real `acme` stack); dev tests set `DESKMATE_POOL_STORE` to a temp dir. Everything is
...
the whole command line for it — even after the script — so a naive `node bin/deskmate.ts … --env-file X`
