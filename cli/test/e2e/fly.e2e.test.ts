...
function flyConfig(label: string): string {
  ...
  return writeConfig(dir, {
    ...
    env: { core: { SNAPSHOT_STORE: "s3", TRANSFER_STORE: "s3", S3_BUCKET: "deskmate-e2e-data", S3_REGION: "auto" } },
  });
}
...
function fly(
  ...
  const identity = "deskmate-v2:e2e-org:acme:deskmate-e2e";
  const defaults = {
    apps: ["deskmate-e2e-core", "deskmate-e2e-web-ui"],
    appOwners: { "deskmate-e2e-core": identity, "deskmate-e2e-web-ui": identity },
  };
...
test("plan (fly) reports MISSING secrets when none are staged, and changes nothing", () => {
  ...
  assert.ok(cmds.some((c) => c[0] === "secrets" && c[1] === "list" && c.includes("deskmate-e2e-core")));
  assert.ok(cmds.some((c) => c[0] === "secrets" && c[1] === "list" && c.includes("deskmate-e2e-web-ui")));
...
test("an explicit Fly --build-from path never falls back to another checkout", () => {
  ...
  const { result } = fly(["plan", "--build-from", "/definitely/not/a/deskmate/checkout"], cfg, { secrets: [] });
  assert.equal(result.code, 1, result.out);
  assert.match(result.out, /--build-from requires a Deskmate checkout/);
});
...
test("--only narrows a fly plan to the named component (and rejects unknown names)", () => {
  ...
  const cmds = fakeFlyCommands(logPath);
  assert.ok(
    cmds.some((c) => c.includes("deskmate-e2e-core")),
    "gated core",
  );
  assert.ok(!cmds.some((c) => c.includes("deskmate-e2e-web-ui")), "did NOT touch web-ui");
...
test("status (fly) shells flyctl status per app", () => {
  ...
  assert.equal(result.code, 0, result.out);
  assert.match(result.out, /deskmate status — deskmate-e2e \(target: fly\)/);
  const cmds = fakeFlyCommands(logPath);
  assert.ok(cmds.some((c) => c[0] === "status" && c.includes("deskmate-e2e-core")));
  assert.ok(cmds.some((c) => c[0] === "status" && c.includes("deskmate-e2e-web-ui")));
});
...
test("logs (fly) shells flyctl logs, and notes that --tail is docker-only", () => {
  ...
  assert.ok(fakeFlyCommands(one.logPath).some((c) => c[0] === "logs" && c.includes("deskmate-e2e-core")));

  const all = fly(["logs"], cfg);
  assert.equal(all.result.code, 0, all.result.out);
  const cmds = fakeFlyCommands(all.logPath);
  assert.ok(cmds.some((c) => c[0] === "logs" && c.includes("deskmate-e2e-core")));
  assert.ok(cmds.some((c) => c[0] === "logs" && c.includes("deskmate-e2e-web-ui")));
});
...
test("down (fly) scales every app to 0", () => {
  ...
  const cmds = fakeFlyCommands(logPath);
  for (const app of ["deskmate-e2e-core", "deskmate-e2e-web-ui"]) {
    assert.ok(
...
test("down (fly) scales a removed-but-running plugin, but never a sibling/deploy/unrelated app", () => {
  ...
  const { result, logPath } = fly(["down"], cfg, {
    apps: [
      ...
      "deskmate-e2e-linear",
      "deskmate-e2e-2-core",
      "deskmate-e2e-2-slack",
      "deskmate-e2e-2-billing",
      "deskmate-e2e-payments",
      "deskmate-e2e-d-acme-blog",
      "other-deploy-core",
    ],
    appOwners: {
      "deskmate-e2e-core": "deskmate-v2:e2e-org:acme:deskmate-e2e",
      "deskmate-e2e-web-ui": "deskmate-v2:e2e-org:acme:deskmate-e2e",
      "deskmate-e2e-linear": "deskmate-v2:e2e-org:acme:deskmate-e2e",
      "deskmate-e2e-2-core": "deskmate-v2:e2e-org:acme:deskmate-e2e-2",
      "deskmate-e2e-2-slack": "deskmate-v2:e2e-org:acme:deskmate-e2e-2",
      "deskmate-e2e-2-billing": "deskmate-v2:e2e-org:acme:deskmate-e2e-2",
    },
  });
  assert.equal(result.code, 0, result.out);
  ...
  for (const app of ["deskmate-e2e-core", "deskmate-e2e-web-ui", "deskmate-e2e-linear"]) {
    assert.ok(scaled.includes(app), `scaled ${app} to 0 (incl. the removed plugin still on Fly)`);
  }
  for (const app of ["deskmate-e2e-2-core", "deskmate-e2e-2-slack", "deskmate-e2e-2-billing"]) {
    assert.ok(!scaled.includes(app), `nested sibling app ${app} must NOT be touched`);
  }
  assert.ok(
    !scaled.includes("deskmate-e2e-payments"),
    "a same-prefix app without our deployment marker must NOT be touched",
  );
  assert.ok(
    !scaled.includes("deskmate-e2e-d-acme-blog"),
    "published deploy apps have their own lifecycle — not torn down",
  );
...
test("down (fly) continues known apps but fails honestly when a same-prefix candidate cannot be inspected", () => {
  ...
  const { result, logPath } = fly(["down"], cfg, {
    apps: ["deskmate-e2e-core", "deskmate-e2e-web-ui", "deskmate-e2e-linear", "deskmate-e2e-payments"],
    appOwners: {
      "deskmate-e2e-core": "deskmate-v2:e2e-org:acme:deskmate-e2e",
      "deskmate-e2e-web-ui": "deskmate-v2:e2e-org:acme:deskmate-e2e",
      "deskmate-e2e-linear": "deskmate-v2:e2e-org:acme:deskmate-e2e",
    },
    failStatusApps: ["deskmate-e2e-payments"],
  });
  assert.equal(result.code, 1, result.out);
  assert.match(
    result.out,
    /down incomplete — could not verify deployment ownership for 1 app; left untouched:[\s\S]*deskmate-e2e-payments/,
  );
  ...
  assert.deepEqual(scaled, ["deskmate-e2e-core", "deskmate-e2e-web-ui", "deskmate-e2e-linear"]);
});
...
test("up --build-only and --image-repo-prefix require --image-label", () => {
  ...
  const repoPrefix = fly(["up", "--only", "core", "--image-repo-prefix", "deskmate"], cfg);
...
test("down (fly) attempts every app, aggregates failures, and exits nonzero", () => {
  ...
  assert.match(result.out, /deskmate-e2e-core.*deskmate-e2e-web-ui/s);
  assert.doesNotMatch(result.out, /down — all apps scaled to 0/);
  const attempted = fakeFlyCommands(logPath)
    .filter((command) => command[0] === "scale")
    .map((command) => command[command.indexOf("-a") + 1]);
  assert.deepEqual(attempted, ["deskmate-e2e-core", "deskmate-e2e-web-ui"]);
});
...
test("down (fly) exits nonzero when flyctl is missing instead of reporting a no-op as success", () => {
  ...
  assert.equal(result.code, 1, result.out);
  assert.match(result.out, /flyctl-not-installed-xyz scale count 0 -a deskmate-e2e-core/);
...
test("fly status with no flyctl installed prints the exact commands to run", () => {
  ...
  assert.match(result.out, /flyctl-not-installed-xyz status -a deskmate-e2e-core/);
});
