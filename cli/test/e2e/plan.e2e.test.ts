...
test("plan reports the docker plan (network, images, env, layer, plugins) and starts nothing", () => {
  ...
  const org = `deskmate-e2e-plan-${process.pid}`;
  try {
    writeConfig(dir, {
      orgId: org,
      target: "docker",
    ...
    assert.match(r.out, new RegExp(`network: deskmate-${org}`));
    assert.match(r.out, /core: image registry\.invalid\/deskmate\/deskmate-core@sha256:a{64}/);
...
test("bare plan --build-from builds from the enclosing deskmate checkout", needsWorktree, () => {
...
test("bare plan --build-from rejects an unrelated enclosing Git repository", () => {
  ...
  try {
    ...
    assert.match(r.out, /not a Deskmate checkout \(missing deploy\/core\/Dockerfile\)/);
  } finally {
    rmDir(dir);
  }
});
...
test("plan honors a host port base from the config", () => {
  ...
  try {
    ...
    const r = runCli(["plan"], { cwd: dir, env: { DESKMATE_BASE_PORT: undefined } });
    assert.equal(r.code, 0, r.out);
...
test("plan honors DESKMATE_BASE_PORT over the config", () => {
  const dir = tmp("plan-portenv");
  try {
    writeConfig(dir, { orgId: "acme", target: "docker", services: ["core"], basePort: 31080 });
    const r = runCli(["plan"], { cwd: dir, env: { DESKMATE_BASE_PORT: "32099" } });
    assert.equal(r.code, 0, r.out);
