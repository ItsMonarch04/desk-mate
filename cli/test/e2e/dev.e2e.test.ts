...
test("normal dev commands delegate to the canonical contributor CLI", needsWorktree, () => {
  const result = runCli(["dev", "frobnicate"], { cwd: repoRoot });
  assert.equal(result.code, 2);
  assert.match(result.out, /usage: dev/);
});

test("dev service names map, dedupe, and reject unknown targets before side effects", needsWorktree, (t) => {
...
test("delegated dev commands reject extra arguments and irrelevant flags before side effects", needsWorktree, () => {
...
test("dev up accepts an org override before entering the canonical pool path", needsWorktree, (t) => {
  const store = tmp("dev-org-pool");
  t.after(() => rmDir(store));
  const result = runCli(["dev", "up", "--org", "beta"], {
    cwd: repoRoot,
    env: { DESKMATE_POOL_STORE: store, DEV_INSTANCE_WAIT: "0" },
  });
