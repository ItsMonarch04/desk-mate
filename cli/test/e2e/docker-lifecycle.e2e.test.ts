...
test(
  ...
  { skip: lifecycleSkip() },
  async (t) => {
    const org = `deskmate-e2e-dl-${process.pid}`;
    ...
    try {
      ...
      await t.test("status enumerates exactly this deployment's containers with ports", () => {
        ...
        assert.equal(r.code, 0, r.out);
        assert.match(r.out, /deskmate status/);
        assert.match(r.out, new RegExp(`deskmate-${org}-core\\b`));
...
test(
  ...
  () => {
    const org = `deskmate-e2e-dl-empty-${process.pid}`;
...
test(
  ...
  { skip: dockerAvailable() ? false : "no Docker daemon reachable" },
  () => {
    const org = `deskmate-e2e-dl-noop-${process.pid}`;
