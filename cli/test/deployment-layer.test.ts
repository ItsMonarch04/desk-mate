...
import { CONFIG_FILENAME, loadConfigInDir, type DeskmateConfig } from "../src/config.ts";
...
test("the deployment layer sync rejects a bundle over the core's 1 MB limit before any request", async () => {
  const dir = mkdtempSync(join(tmpdir(), "deskmate-layer-big-"));
  try {
...
test("a missing sandbox directory skips sync instead of replacing the deployed layer with empty", async (t) => {
  const dir = mkdtempSync(join(tmpdir(), "deskmate-layer-missing-"));
  const lines: string[] = [];
...
function makeConfig(publicUrl: string): DeskmateConfig {
  return {
    contract: 1,
    orgId: "acme",
    publicUrl,
...
test("the docker sync PUTs the bundle to the base port with verifiable v0 HMAC signing headers", async () => {
  const dir = mkdtempSync(join(tmpdir(), "deskmate-layer-sync-"));
  ...
  try {
    ...
    await withEnv({ CORE_SIGNING_SECRET: "wrong-ambient-secret", DESKMATE_BASE_PORT: String(port) }, () =>
      syncDeploymentLayer({
        config: makeConfig("http://example.invalid"),
        transport: dockerDeploymentLayerTransport,
        configDir: dir,
...
test("conformance passes against a live core: base-port override, signed request, canonical hash + descriptors", async () => {
  const dir = mkdtempSync(join(tmpdir(), "deskmate-conf-"));
  const captured: CapturedRequest[] = [];
  const bundle = (() => {
    writeLayer(dir);
  ...
  try {
    ...
    await withEnv({ CORE_SIGNING_SECRET: SECRET, DESKMATE_BASE_PORT: String(port) }, async () => {
...
test("a successful memory-backed sync warns that the layer will not survive restart", async (t) => {
  ...
  try {
    writeLayer(dir);
    await withEnv({ CORE_SIGNING_SECRET: SECRET, DESKMATE_BASE_PORT: String(port) }, () =>
      syncDeploymentLayer({
        config: makeConfig("http://example.invalid"),
        transport: dockerDeploymentLayerTransport,
        configDir: dir,
...
test("allowUnavailable swallows an unreachable core but NOT a local config error", async () => {
  ...
  try {
    ...
    await withEnv({ CORE_SIGNING_SECRET: undefined, DESKMATE_BASE_PORT: String(port) }, () =>
      assert.rejects(
        () =>
          syncDeploymentLayer({
            config: makeConfig("http://example.invalid"),
...
test("fly sync succeeds on the response marker, piping the exact bundle over stdin", async () => {
  const dir = mkdtempSync(join(tmpdir(), "deskmate-layer-fly-"));
  try {
    writeLayer(dir);
    ...
    const bin = fakeFly(
      ...
      [
        ...
        `console.log('DESKMATE_LAYER_RESPONSE=' + JSON.stringify({ status: 200, body: JSON.stringify({ version: 7, contentHash: "abcdef123456" }) }));`,
      ].join("\n"),
    );
...
test("a 202 degraded response is accepted and warns with the core's persisted-but-partial message", async (t) => {
  const dir = mkdtempSync(join(tmpdir(), "deskmate-layer-fly-degraded-"));
  const warnings: string[] = [];
  ...
  try {
    ...
    const bin = fakeFly(
      dir,
      `console.log('DESKMATE_LAYER_RESPONSE=' + JSON.stringify({ status: 202, body: JSON.stringify({ status: "degraded", version: 8, contentHash: "abc", durable: true, message: "skill collision" }) }));`,
    );
    await withEnv({ FLY_BIN: bin }, () => syncDeploymentLayer(flySyncOpts(dir)));
...
test("a remote-script error (signing secret missing on core) is NOT deferrable as core-unreachable", async () => {
  ...
  try {
    ...
    const bin = fakeFly(
      dir,
      `console.log('DESKMATE_LAYER_ERROR=' + JSON.stringify({ message: "CORE_SIGNING_SECRET is not set on core" }));`,
    );
...
test("a remote connection failure (core process down inside the VM) IS deferrable", async () => {
  ...
  try {
    ...
    const bin = fakeFly(
      dir,
      `console.log('DESKMATE_LAYER_ERROR=' + JSON.stringify({ message: "fetch failed", code: "ECONNREFUSED" }));`,
    );
...
test("junk files (.DS_Store, Thumbs.db, AppleDouble) are excluded from the bundle", () => {
  const dir = mkdtempSync(join(tmpdir(), "deskmate-layer-junk-"));
  try {
    writeLayer(dir);
...
test("non-junk tools entries fail loudly unless they are directories with tool.json", () => {
  const dir = mkdtempSync(join(tmpdir(), "deskmate-layer-tools-shape-"));
  try {
...
test("a core with no durable layer record bootstraps as empty regardless of its live source", async () => {
  const dir = mkdtempSync(join(tmpdir(), "deskmate-layer-boot-"));
  const captured: CapturedRequest[] = [];
...
test("a durable record whose bundle is missing still fails the read", async () => {
  const dir = mkdtempSync(join(tmpdir(), "deskmate-layer-durable-"));
  const captured: CapturedRequest[] = [];
  const { server } = await startCoreStub(
    () => ({
