...
import { CONFIG_FILENAME, loadConfigAt, type DeskmateConfig } from "../src/config.ts";
...
test("sandbox publish pins the base, reads the pushed digest, and records the pin in the config", () => {
  const dir = mkdtempSync(join(tmpdir(), "deskmate-publish-"));
...
`,
    ...
    assert.equal(
      ...
      `registry.invalid/deskmate/deskmate-sandbox-base@sha256:${"a".repeat(64)}`,
...
test("sandbox publish --from a Fly registry tag pins by pull, never by registry inspect", () => {
  const dir = mkdtempSync(join(tmpdir(), "deskmate-publish-from-fly-"));
  const priorPath = process.env.PATH;
  const log = console.log,
    warn = console.warn;
  console.log = (): void => {};
...
test("sandbox publish rejects a custom Dockerfile whose final stage forces a non-amd64 platform", () => {
  const dir = mkdtempSync(join(tmpdir(), "deskmate-publish-platform-"));
  const priorPath = process.env.PATH;
  const log = console.log,
    warn = console.warn;
  console.log = (): void => {};
...
test("sandbox publish ignores FROM text inside heredocs when checking the real final stage", () => {
  const dir = mkdtempSync(join(tmpdir(), "deskmate-publish-heredoc-platform-"));
  const priorPath = process.env.PATH;
  const log = console.log,
    warn = console.warn;
  console.log = (): void => {};
...
test("sandbox publish recognizes ONBUILD heredocs with concatenated quoted delimiters", () => {
  const dir = mkdtempSync(join(tmpdir(), "deskmate-publish-onbuild-heredoc-"));
  const priorPath = process.env.PATH;
  const log = console.log,
    warn = console.warn;
  console.log = (): void => {};
...
test("sandbox publish ignores heredoc aliases when identifying mutable external bases", () => {
  const dir = mkdtempSync(join(tmpdir(), "deskmate-publish-heredoc-base-"));
  const priorPath = process.env.PATH;
  const log = console.log,
    warn = console.warn;
  console.log = (): void => {};
...
test("sandbox publish fails closed on an unparseable FROM before Docker side effects", () => {
  const dir = mkdtempSync(join(tmpdir(), "deskmate-publish-from-"));
  const priorPath = process.env.PATH;
  const log = console.log,
    warn = console.warn;
  console.log = (): void => {};
...
test("sandbox publish accepts FROM scratch without trying to resolve a nonexistent base digest", () => {
  const dir = mkdtempSync(join(tmpdir(), "deskmate-publish-scratch-"));
  const priorPath = process.env.PATH;
  const log = console.log,
    warn = console.warn;
  console.log = (): void => {};
...
const AWS_BLOCK: DeskmateConfig["aws"] = {
  ...
  services: { core: { ecrRepository: "deskmate-core", ecsService: "acme-core", cpu: 2048, memory: 4096 } },
};
...
test("the AWS provider rejects sandbox registry publication before build or registry work, --app included", async () => {
  const dir = mkdtempSync(join(tmpdir(), "deskmate-publish-aws-"));
  const priorPath = process.env.PATH;
  try {
    const configPath = join(dir, CONFIG_FILENAME);
...
test("AWS sandbox publish preconditions run before the build: frozen override and missing manifest", async () => {
  const dir = mkdtempSync(join(tmpdir(), "deskmate-publish-aws-precheck-"));
  const priorPath = process.env.PATH;
...
test("sandbox publish on AWS with sandbox.backend sprites proceeds to the layer build (operator-hosted sandbox image)", async () => {
  const dir = mkdtempSync(join(tmpdir(), "deskmate-publish-aws-fly-"));
  const aws = fakeAwsBin(dir, {
    id: "m1",
...
test("pinnedByDigest verifies digest-form refs but warns and proceeds when the registry inspect fails", () => {
  const dir = mkdtempSync(join(tmpdir(), "deskmate-pinned-digest-"));
  const emptyPath = mkdtempSync(join(tmpdir(), "deskmate-pinned-no-docker-"));
  const priorPath = process.env.PATH;
