...
them locally, Fly runs them as Fly apps with Fly Machines for agent computers, AWS
runs digest-pinned ARM64 tasks on ECS Fargate with Lambda MicroVM agent computers, and
GCP runs them on Cloud Run with interim Fly Sprites agent sandboxes.
...
the config and the image pin in the config (docker/fly/gcp) or the durable AWS deployment
...
```text
init [dir] [--org id] [--target docker|fly|aws|gcp]
check [--json] [--live]
doctor
...
the AWS CLI, the Google Cloud CLI, and Git. Terraform is operator-run against the module
generated by `init`.
