...
The root object requires `contract: 1`, `orgId`, `publicUrl`, `target`, and `services` including `core`. Docker, Fly, and GCP require `sandbox.app`; GCP uses Fly Sprites as its interim agent sandbox. On AWS the sandbox substrate is an explicit choice: omitting the `sandbox` block runs named Lambda MicroVM images; declaring one requires `sandbox.backend` — `"sprites"` boots the operator-published layer image in `sandbox.app`, `"aws"` states the MicroVM default in the file. Unknown contract majors fail closed. `target` is `docker`, `fly`, `aws`, or `gcp`.
...
`init` renders the set as `.env.example`; that file has names and descriptions, never values, and is not an input to deployment. Operators place values in gitignored `.env`. Docker reads the file locally. `deskmate secrets push` uploads supplied operator-managed values to Fly secrets, AWS Secrets Manager, or GCP Secret Manager without printing them. Terraform owns `DATABASE_URL` on AWS and GCP because it owns the database. `doctor` treats missing and placeholder required values as failures and reports absent optional plugin secrets without blocking deployment.
...
| --------------------------------- | -------------- | -------------------------------------------------------------------------------------- |
| `config.v1`                       | ENFORCED       | `loadConfigAt`, `deskmate check`                                                       |
| `config.no-secret-values`         | ENFORCED       | `deskmate check`                                                                       |
| `secrets.computed-set`            | ENFORCED       | typed schema, `deskmate check`                                                         |
...
| `runtime.layer-resolved`          | ENFORCED       | deployment-layer store/API and core PUT validation, `deskmate conformance` cross-check |
| `aws.rendered-task`               | ENFORCED       | `renderTaskDefinition`, AWS `plan`/`up`, digest and task-diff tests                    |
| `aws.live-drift`                  | ENFORCED       | `deskmate check --live`, bidirectional task/environment/secret/release checks          |
| `sandbox.egress`                  | VALIDATED-ONLY | wildcard/host warnings in `deskmate check`; no runtime enforcement claimed             |
