...
- real LLM: needs a model credential for the harness you run. Core supports several
  (`HARNESS=opencode|codex|claude`); the launcher picks one from the credentials it
...
`~/.config/deskmate/dev.env`, your login shell (for a model credential exported there), and this
worktree's `.env` (seeded from the main checkout in linked worktrees). Slack pool tokens
default to `~/.config/deskmate/slack-pool`.
