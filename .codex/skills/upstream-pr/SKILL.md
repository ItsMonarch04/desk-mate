---
name: upstream-pr
description: Send a change to upstream Deskmate without leaking organization-specific context. Use when asked to "upstream this", "open a PR against Deskmate", "contribute this back", or when a fix made in a private fork belongs in core deskmate.
---

# upstream-pr

An organization customizes Deskmate from a private fork: a standalone private repository in
...
Upstream Deskmate is shared with organizations other than yours, and anything pushed there is
...
`update-deskmate` skill for syncing the other direction.
...
`<org>/deskmate-<something>` in the same GitHub organization as the source is still a private
fork.
...
when it is a fix or capability in core Deskmate that any deployment would want. It does not
...
by reading your own layer: `deskmate.config.jsonc` has the org slug and public URL host,
...
If you have write access to upstream Deskmate, push the topic branch there and open the PR inside
that repository:
...
If you do not, keep a separate public GitHub fork of Deskmate for contributions and push the
...
next `update-deskmate` sync, and committing it in both places guarantees a conflict.
