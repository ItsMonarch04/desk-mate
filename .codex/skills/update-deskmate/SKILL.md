---
name: update-deskmate
description: Update a private Deskmate fork by merging upstream Deskmate into it and opening a sync PR. Use when asked to "update Deskmate", "sync from upstream", "pull in the latest Deskmate".
---

# update-deskmate

An organization customizes Deskmate from a private fork: a standalone private repository whose
history begins as a clone of Deskmate, with everything organization-specific confined to
...
This skill keeps a private fork current. It merges upstream Deskmate in and opens the
resulting sync PR.
...
Upstream changing the layer contract does not conflict either; it surfaces as a `deskmate check`
failure, covered below. Every conflict in a sync therefore means the private fork edited a
file that exists in upstream Deskmate, which the layer boundary exists to prevent. For each such
...
- The edit belongs in core Deskmate for everyone. Resolve in whichever direction keeps the tree
  working, then use the `upstream-pr` skill to send it to Deskmate so the next sync stops
  conflicting on it.
...
for the old one. Check each organization layer with the in-tree CLI (`npm exec deskmate` does not
work in a source checkout; the workspace symlink points at `cli/`, which is unbuilt):

```bash
node cli/bin/deskmate.ts check --config deploy/layers/<org>/deskmate.config.jsonc
```
...
gh pr create --repo <private-fork> --base main \
  --title "Sync upstream deskmate through <short-sha>" \
...
against Deskmate, and `gh pr edit 1` overwrites whatever PR is number 1 in the source repository.
