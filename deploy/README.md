...
The one exception is [`layers/`](./layers/), which is empty in Deskmate itself. A private
...
The supported command sequence is generated by `deskmate init` and the selected
provider reference under [`.codex/skills/deploy-deskmate/references/`](../.codex/skills/deploy-deskmate/references/).
...
By default the OIDC provider is the `auth` service Deskmate deploys for itself: a
...
the authenticated admin connector URL returned by `deskmate outputs`. Secrets are
