...
```text
deploy/layers/<org>/
  deskmate.config.jsonc          the deployment config; committed, no secret values
...
Run the CLI from the tree as shown. `npm exec deskmate` does not work in a source checkout
...
Nothing under `deploy/layers/` may reach upstream Deskmate: not the config, not the sandbox
