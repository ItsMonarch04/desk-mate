...
test("admin shell uses the Deskmate identity with org-injectable branding", () => {
  assert.match(html, /<title>Deskmate Admin<\/title>/);
...
test("connector setup uses the live catalog and shows exact provider and callback links", () => {
  ...
  assert.match(html, /encrypted in durable\s+storage/);
});
