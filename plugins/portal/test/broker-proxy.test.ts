...
test("the broker's sign-in pages are reachable without a session", async () => {
  const page = await fetch(`${base}/idp/authorize?client_id=deskmate-portal&state=s`);
  assert.equal(page.status, 200);
  assert.equal(await page.text(), "<form>broker</form>");
  assert.equal(
    seen.at(-1)!.url,
    "/authorize?client_id=deskmate-portal&state=s",
...
test("the portal stamps the client address and refuses to relay a spoofed one", async () => {
  await fetch(`${base}/idp/authorize?x=1`, { headers: { "x-deskmate-client-ip": "9.9.9.9" } });
  const relayed = seen.at(-1)!.headers["x-deskmate-client-ip"];
  ...
  assert.notEqual(
    seen.at(-1)!.headers["x-deskmate-client-ip"],
    "203.0.113.7",
...
test("isPrivateNetworkUrl admits only unroutable hosts", () => {
  for (const url of [
    "http://deskmate-auth.internal:8080",
    "http://deskmate-web-ui.flycast",
    "http://localhost:8099",
    "http://127.0.0.1:8099",
    "http://10.1.2.3:8080",
