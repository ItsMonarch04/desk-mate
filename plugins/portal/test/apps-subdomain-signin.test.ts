...
process.env.PORTAL_COOKIE_DOMAIN = "deskmate.example.com";
process.env.PORTAL_APPS_DOMAIN = "apps.deskmate.example.com";
...
const APP_URL = "https://contracts.apps.deskmate.example.com/?dpl_signin=1";
...
test("the session cookie is scoped to the parent domain so app subdomains receive it", async () => {
  ...
  assert.match(session, /Domain=deskmate\.example\.com/);
  assert.ok(
...
test("sign-in returns the visitor to the app they came from, not the chat home", async () => {
  ...
  assert.equal(login.status, 302);
  assert.equal(login.headers.get("location"), "https://contracts.apps.deskmate.example.com/?dpl_signin=1");
});

test("only single-label app hosts under the apps domain survive the returnTo", async () => {
  const rejected = [
    "https://evil.com/",
    "http://contracts.apps.deskmate.example.com/",
    "https://a.b.apps.deskmate.example.com/",
    "https://notapps.deskmate.example.com/",
    "https://contracts.apps.deskmate.example.com.evil.com/",
    "https://user:pw@contracts.apps.deskmate.example.com/",
  ];
...
test("the cookie-domain boot check matches the browser's suffix rule", () => {
  assert.equal(hostIsWithinDomain("deskmate.example.com", "deskmate.example.com"), true);
  assert.equal(hostIsWithinDomain("apps.deskmate.example.com", "deskmate.example.com"), true);
  assert.equal(hostIsWithinDomain("contracts.apps.deskmate.example.com", "deskmate.example.com"), true);
  assert.equal(hostIsWithinDomain("DESKMATE.EXAMPLE.COM", ".deskmate.example.com"), true);
  assert.equal(hostIsWithinDomain("notdeskmate.example.com", "deskmate.example.com"), false);
  assert.equal(hostIsWithinDomain("deskmate.example.com.evil.com", "deskmate.example.com"), false);
  assert.equal(hostIsWithinDomain("example.com", "deskmate.example.com"), false);
  assert.equal(hostIsWithinDomain("", "deskmate.example.com"), false);
  assert.equal(hostIsWithinDomain("deskmate.example.com", ""), false);
});
...
test("logout clears the cookie at both scopes", async () => {
  ...
  assert.ok(
    cookies.some((c) => /Domain=deskmate\.example\.com/.test(c)),
