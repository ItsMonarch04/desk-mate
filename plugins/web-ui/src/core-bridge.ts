import { createAssistantMessageEventStream, type AssistantMessageEventStream } from "./assistant-event-stream.ts";
import type { AssistantMessage, Context, Model } from "./model-types.ts";
import { zeroUsage } from "./model-types.ts";
import type { Agent, StreamFn } from "./agent.ts";
import type { AgentMessage, Attachment } from "./model-types.ts";
...
function baseAssistant(model: Model): AssistantMessage {
  return {
    role: "assistant",
...
async function latestUserTurn(agent: Agent): Promise<{ text: string; attachments: CoreAttachment[] }> {
  const messages = agent.state.messages as Array<AgentMessage & { attachments?: Attachment[] }>;
...
function attachmentBytes(a: Attachment): Uint8Array {
...
async function toCoreAttachment(a: Attachment): Promise<CoreAttachment> {
...
export function makeCoreStreamFn(
  ...
  const fn = (model: Model, _context: Context, options?: { signal?: AbortSignal }): AssistantMessageEventStream => {
...
export function entriesToMessages(entries: SessionEntry[], model: Model): AgentMessage[] {
...
export function attachPendingApprovals(messages: AgentMessage[], approvals: PendingApproval[], model: Model): void {
