...
/** Mirrors core's DEFAULT_HARNESS_ID: the harness a deployment falls back to. */
const DEFAULT_HARNESS_ID = "claude";
import { getBaseModel } from "./fast-mode.ts";
...
function buildOptions(
  ...
  return harnessId === DEFAULT_HARNESS_ID ? buildOptions(DEFAULT_PICKER_MODEL_IDS, harnessId, qualified, catalog) : [];
}
...
export function transcriptModel(): Model {
...
export function harnessSupportsEffort(harnessId: string): boolean {
  return harnessId === "codex" || harnessId === "claude";
}
...
export function defaultEffortForModel(model: Model): EffortLevel {
