...
const audienceFor = (purpose: TokenPurpose): string => `deskmate-auth:${purpose}`;
...
export class TokenSigner {
  ...
  private keyFor(purpose: TokenPurpose): Uint8Array {
    ...
    const derived = new Uint8Array(createHmac("sha256", this.secret).update(`deskmate-auth.${purpose}.v1`).digest());
