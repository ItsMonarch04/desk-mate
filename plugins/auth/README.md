...
Every value below is set by `deskmate` from the deployment config and the secret
...
| `AUTH_CLIENT_SECRET`, `AUTH_TOKEN_SECRET`, `AUTH_SIGNING_JWK`                   | generated by `deskmate setup`          |
...
`deskmate doctor` proves the Resend key is accepted, or that the SMTP relay is
