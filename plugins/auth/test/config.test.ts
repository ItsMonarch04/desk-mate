...
test("malformed allowlists and senders are refused", () => {
  ...
  assert.match(problemsFor({ AUTH_EMAIL_FROM: "deskmate <not-an-address>" }), /verified sender address/);
});
...
test("senderAddress unwraps a display name", () => {
  assert.equal(senderAddress("deskmate <no-reply@example.com>"), "no-reply@example.com");
