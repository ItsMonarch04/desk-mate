...
async function findPostedByKey(
  ...
  do {
    ...
    const hit = (page.messages ?? []).find(
      (m) =>
        m.metadata?.event_type === "deskmate_delivery" && m.metadata.event_payload?.idempotency_key === idempotencyKey,
    );
...
export async function postWithVerify(
  ...
  args.metadata = { event_type: "deskmate_delivery", event_payload: { idempotency_key: idempotencyKey } };
  if (opts?.verifyFirst) {
