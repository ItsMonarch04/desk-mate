...
import { DEFAULT_HARNESS_ID, isHarnessId } from "../model/harness-ids.ts";
      ...
      if (req.surface === "web") {
        ...
        const targetScope =
          req.conversation.kind === "dm"
            ? scopeId("personal", actor.id)
            : scopeId(req.conversation.kind, req.conversation.channelRef ?? threadRef);
        const fallbackHarness = isHarnessId(deps.harnessId) ? deps.harnessId : DEFAULT_HARNESS_ID;
        ...
        if (deps.modelCredentials || deps.providerKeys) {
