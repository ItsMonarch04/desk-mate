...
import { modelServiceable, modelSupportedByHarness } from "../../model/capabilities.ts";
import { resolveModel, SELECTABLE_BASE_MODELS } from "../../model/catalog.ts";
import { defaultModelForHarness } from "../../model/defaults.ts";
import { DEFAULT_HARNESS_ID, HARNESS_IDS, isHarnessId } from "../../model/harness-ids.ts";
import { ALL_PROVIDERS_AVAILABLE, modelProviderAvailabilityFor } from "../../model/providers.ts";
...
export const ADMIN_RESOURCES: readonly AdminResource[] = [
  ...
  },
  {
    ...
    apply: async (ctx, _actor, scope) => {
      ...
      const unserviceable = (harness: string): { error: string } | null =>
        modelId && !modelServiceable(modelId, modelProviderAvailabilityFor(harness, configuredKeys))
          ? {
      ...
      if (!modelId) await ctx.deps.config!.setRuntimeSelectionLatest(scope, null);
        ...
        const harnessId = isHarnessId(ctx.deps.harnessId) ? ctx.deps.harnessId : DEFAULT_HARNESS_ID;
  ...
  },
  {
    ...
    apply: async (ctx, _actor, scope) => {
      ...
      const approved = (await ctx.deps.config!.getApprovedHarnessesDurable()) ?? [
        ctx.deps.harnessId ?? DEFAULT_HARNESS_ID,
      ];
  ...
  },
  {
    ...
    apply: async (ctx, _actor, scope) => {
      ...
      if (modelId && !resolveModel(modelId)) return { error: `unknown model id: ${modelId}` };
      const configuredKeys = ctx.deps.providerKeys ?? ALL_PROVIDERS_AVAILABLE;
      const providers = modelProviderAvailabilityFor(ctx.deps.harnessId ?? DEFAULT_HARNESS_ID, configuredKeys);
