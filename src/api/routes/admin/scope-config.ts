...
export async function getScopeConfig(ctx: ApiCtx): Promise<void> {
  ...
  const providersFor = (harnessId: string) => modelProviderAvailabilityFor(harnessId, configuredKeys);
  const catalog =
  ...
  return sendJson(res, 200, {
    ...
    baseModelDefault: defaultModelForHarness(deps.harnessId ?? DEFAULT_HARNESS_ID, deps.baseModelDefault),
    baseModelOptions: modelsFor(deps.harnessId ?? DEFAULT_HARNESS_ID),
    harnessDefault: deps.harnessId ?? DEFAULT_HARNESS_ID,
    ...
    browseModelOptions: SELECTABLE_BASE_MODELS.filter((m) =>
      modelServiceable(m.id, providersFor(deps.harnessId ?? DEFAULT_HARNESS_ID)),
    ),
    egressEnforcement: {
