...
import { DESKMATE_VERSION } from "./version.ts";
import { errMessage } from "./util/errors.ts";
import { defaultModelForHarness } from "./model/defaults.ts";
import { modelProviderAvailabilityFor } from "./model/providers.ts";
...
server.listen(config.port, () => {
  console.log(
    `[deskmate] v${DESKMATE_VERSION} listening on :${config.port} (org=${config.orgId}, store=${config.sessionStore}, ` +
...
if (config.backgroundWorkEnabled) {
  ...
  console.log("[deskmate] background work disabled; scheduler and runtime loops will not start");
}
...
const slackRuntime = createSlackRuntimeReconciler({
  ...
  onError: (error) => console.error(`[deskmate] slack plugin reconciliation failed: ${errMessage(error)}`),
});
slackRuntime.start();
...
function shutdown(signal: string): void {
  ...
  shuttingDown = true;
  console.log(`[deskmate] ${signal} received, shutting down`);
  void slackRuntime.stop().catch((e: unknown) => console.error("[deskmate] slack plugin stop failed:", errMessage(e)));
  built.scheduler.stop();
  built.deploymentLayerRefresh.stop();
  server.close();
  server.closeIdleConnections();
  stopWithBackstop(built.runtime, config.shutdownDrainMs, "deskmate", () => server.closeAllConnections());
}
