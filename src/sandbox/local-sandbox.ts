...
const DEFAULT_LOCAL_SANDBOX_IMAGE = "deskmate-sandbox-local:latest";
const HOME_DIR = "/root";
const WORKSPACE_BASENAME = "workspace";
const AGENT_PORT = 8080;
const RO_LAYERS_TAR = ".ro-layers.tar";
const RO_LAYERS_MANIFEST = ".ro-layers.manifest";
const FINGERPRINT_LABEL = "deskmate.sandbox-fingerprint";
...
export const localContainerName = (scopeId: string): string => `deskmate-sbx-${localSlug(scopeId)}`;
export const localVolumeName = (scopeId: string): string => `deskmate-home-${localSlug(scopeId)}`;
export const localNetworkName = (containerName: string): string =>
  `deskmate-net-${containerName.replace(/^deskmate-(sbx|scratch)-/, "")}`;
const localScratchName = (key: string): string => `deskmate-scratch-${localSlug(key)}`;
...
export function createLocalSandbox(workspace: WorkspaceStore, opts: LocalSandboxOptions = {}): Sandbox {
  ...
  async function runContainer(name: string, scope: string | undefined, withVolume: boolean): Promise<void> {
    ...
    const args = [
      ...
      "--label",
      "deskmate.sandbox=1",
      ...(scope ? ["--label", `deskmate.scope=${scope}`] : []),
      "--label",
      `deskmate.org=${configOrgId()}`,
      "--label",
      "agent_env=dev",
      "--network",
      net,
