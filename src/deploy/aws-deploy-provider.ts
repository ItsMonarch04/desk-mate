...
const dataTarPath = (): string =>
  `/tmp/deskmate-data-${Date.now().toString(36)}-${Math.random().toString(36).slice(2, 8)}.tar`;
const DATA_DB = `${DATA_DIR}/app.db`;
const LITESTREAM_CONFIG = "/etc/deskmate-litestream.yml";
const LITESTREAM_ENV = "/etc/deskmate-litestream.env";
const LITESTREAM_LOG = "/tmp/deskmate-litestream.log";
...
const READY_PATH = `${APP_DIR}/.deskmate-ready`;
const PID_PATH = "/tmp/deskmate-app.pid";
const LOG_PATH = "/tmp/deskmate-app.log";
...
export function createAwsDeployProvider(opts: AwsDeployProviderOptions): DeployProvider {
  ...
  async function checkoutBundle(id: string, endpoint: string, bundle: Uint8Array, commit: string): Promise<void> {
    const bundlePath = `/tmp/deskmate-deploy-${commit.slice(0, 12)}.bundle`;
    ...
    const script = [
      ...
      "git clean -ffd -e .deskmate-ready",
