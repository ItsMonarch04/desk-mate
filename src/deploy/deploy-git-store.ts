...
const DEFAULT_REPO_ROOT = join(tmpdir(), "deskmate-deploy-git");
const GIT_AUTHOR = ["-c", "user.name=Deskmate", "-c", "user.email=deployments@deskmate.local"];
const ARCHIVE_ETAG_FILE = "deskmate-archive-etag";
...
export function createDeployGitStore(opts: DeployGitStoreOptions = {}): DeployGitStore {
  ...
  async function restoreArchive(deploymentId: string, archive: DeployGitArchive): Promise<string> {
    ...
    const tmp = await mkdtemp(join(tmpdir(), "deskmate-deploy-git-bundle-"));
    try {
  ...
  async function checkoutForCommit(
    ...
    const workdir = await mkdtemp(join(tmpdir(), "deskmate-deploy-git-"));
