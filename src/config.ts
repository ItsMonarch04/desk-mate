...
import { defaultModelForProvider } from "./model/defaults.ts";
import {
  isModelProvider,
  MODEL_PROVIDERS,
  type ModelProvider,
...
function harnessEnvStrict(value: string | undefined): Config["harness"] {
  ...
  if (harness === "mock" || harness === "opencode" || harness === "codex" || harness === "claude") return harness;
  throw new Error(
    `HARNESS=${JSON.stringify(value)} is not recognized — use mock, opencode, codex, or claude, or unset it.`,
  );
}
...
function secretsBackendEnvStrict(value: string | undefined, prefix: string): Config["secretsBackend"] {
  ...
  if (backend === "aws") {
    if (!prefix)
      throw new Error(
        "SECRETS_BACKEND=aws requires a non-empty SECRETS_PREFIX (e.g. deskmate-prod-) so lookups can't collide across stacks sharing an account.",
      );
    return backend;
  }
...
export function loadConfig(env: NodeJS.ProcessEnv = process.env): Config {
  ...
  if (env.NODE_ENV === "production" && harnessEnvStrict(env.HARNESS) === "mock") {
    console.warn(
      `[config] HARNESS is ${env.HARNESS?.trim() ? '"mock"' : "unset, which means mock"} in production — this deployment answers every message with canned text and calls no model provider. Set HARNESS=claude to run real agent turns.`,
    );
  }
  ...
  return {
    ...
    ...(env.AGENT_MODEL ? { modelId: env.AGENT_MODEL } : {}),
    ...(env.OPENCODE_MODEL || env.AGENT_MODEL ? { opencodeModel: env.OPENCODE_MODEL || env.AGENT_MODEL } : {}),
