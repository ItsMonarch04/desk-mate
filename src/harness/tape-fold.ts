...
/**
 * A folded tape may only be replayed to the harness that wrote it: transcript formats differ, so
 * seeding one harness with rows another recorded produces a malformed history. Rows written before
 * a harness stamped itself carry no id and are treated as belonging to whoever reads them.
 */
export function singleHarnessTape(rows: readonly TapeRecord[]): boolean {
  const seen = new Set<string>();
  for (const row of rows) {
    if (row.kind === "message" && row.harness !== undefined) seen.add(row.harness);
  }
  return seen.size <= 1;
}
