...
import { contextTokenBudgetForModel, modelSupportedByHarness, modelSupportsFastMode } from "../model/capabilities.ts";
import { DEFAULT_AGENT_MODEL_ID } from "../model/defaults.ts";
...
import { coreToolOptions, createCoreTools, type CoreToolsOptions, type ToolContextRef } from "./core-tools.ts";
...
function toolOptions(opts: ClaudeHarnessOptions, turn?: HarnessTurnInput): CoreToolsOptions {
  return {
...
function asTools(ref: ToolContextRef, options: CoreToolsOptions): BridgedTool[] {
  return createCoreTools(ref, options) as unknown as BridgedTool[];
}
...
export function claudeReplayTranscript(messages: readonly PiReplayMessage[]): string {
  ...
  return [
    "## Prior conversation (replayed from Deskmate's durable session log)",
...
export function createClaudeHarness(opts: ClaudeHarnessOptions = {}): Harness {
  ...
  const runPrompt = async (turn: HarnessTurnInput, toolsEnabled = true): Promise<HarnessTurnResult> => {
    ...
    const jail = mkdtempSync(join(tmpdir(), "deskmate-claude-"));
    ...
    const bridgedNames = bridged.map((definition) => `mcp__deskmate__${definition.name}`);
    const childToolNames = bridged
      .filter((definition) => CHILD_TOOL_NAMES.has(definition.name))
      .map((definition) => `mcp__deskmate__${definition.name}`);
    ...
    const server = createSdkMcpServer({ name: "deskmate", version: "1", tools: definitions, alwaysLoad: true });
                          ...
                          : {
                              hookSpecificOutput: {
                                ...
                                permissionDecisionReason:
                                  "Only the research, code, and consult Deskmate subagents are available.",
                              },
                            },
                    ],
                  },
