...
import { defineTool, type AnyToolDefinition } from "./tool-definition.ts";
...
export type CoreToolOptions = Omit<CoreToolsOptions, "readOnly" | "surfaceTools" | "surfaceName">;
...
export function createCoreTools(ref: ToolContextRef, opts?: CoreToolsOptions): AnyToolDefinition[] {
  ...
  if (reachExec) {
    execute = defineTool({
      ...
      parameters: z.object({
        ...executeBaseParams,
        scope: z.string().describe(reachScopeDescription).optional(),
        ...(scratchExec || ownerAuthExec
          ? {
              durable: z.boolean().describe(`${DURABLE_PARAM_DESC} Ignored when reaching a room.`).optional(),
            }
          : {}),
      }),
    ...
    execute = defineTool({
      ...
      parameters: z.object({
        ...
        scope: z
          .union([
            z.literal("scoped"),
            ...(scratchExec ? [z.literal("scratch")] : []),
            ...(ownerAuthExec ? [z.literal("owner")] : []),
          ])
          .describe(
          ...
          .optional(),
        durable: z.boolean().describe(DURABLE_PARAM_DESC).optional(),
      }),
    ...
    execute = defineTool({
      ...
      parameters: z.object(executeBaseParams),
  ...
  const write = defineTool({
    ...
    parameters: z.object({
      ...
      data: z
        .string()
        .describe("File contents. Omit to only (re)share a file that already exists at `path`.")
        .optional(),
      share: z
        .array(
          z.object({
            scope: z.string().describe('Who to share with: "org", personal:<userId>, channel:<id>, or team:<id>.'),
            permission: z
              .union([z.literal("read"), z.literal("write")])
              .describe("read = view (default), write = view + manage.")
              .optional(),
          }),
        )
        .describe("Grant other scopes access to this file (Google-Docs-style ACL grants).")
        .optional(),
    }),
    async execute(callId, params) {
      const tc = ref.current;
  ...
  const publish = defineTool({
    ...
    parameters: z.object({
      dir: z.string().describe("Workspace directory to publish (default: the whole tree).").optional(),
      entrypoint: z
        .string()
        .describe('Command the container runs, relative to the app root, e.g. "node server.js".')
        .optional(),
      name: z
        .string()
        .describe("Friendly, globally-unique handle → link is /d/<name>/. Lowercase letters/digits/hyphens.")
        .optional(),
      renameFrom: z.string().describe("Rename the deployment currently named this to `name`.").optional(),
      env: z.record(z.string(), z.string()).describe("Env vars baked into the immutable version.").optional(),
      rollbackTo: z.int().describe("Flip the deployment named `name` back to this version number.").optional(),
      share: z
        .array(
          z.object({
            scope: z.string().describe("Scope id to share with, e.g. personal:<id> or org:<id>."),
            permission: z.union([z.literal("read"), z.literal("write")]),
          }),
        )
        .describe("Grant other scopes access (read = reach, write = manage).")
        .optional(),
    }),
    async execute(callId, params) {
      const tc = ref.current;
  ...
  const memory = defineTool({
    ...
    parameters: z.object({
      action: z
        .union([z.literal("search"), z.literal("remember"), z.literal("read"), z.literal("rewrite")])
        .describe("search remembered facts, remember new facts, read the whole notebook, or rewrite (replace) it."),
      query: z
        .string()
        .describe("search only: words to look for among your remembered facts (all must match).")
        .optional(),
      limit: z.int().describe("search only: max facts to return (default 20).").optional(),
      facts: z
        .array(z.string())
        .describe(
      ...
      content: z
        .string()
        .describe("rewrite only: the FULL new notebook content (replaces the whole notebook).")
        .optional(),
    }),
    async execute(callId, params) {
      const tc = ref.current;
  ...
  const history = defineTool({
    ...
    parameters: z.object({
      query: z.string().describe("Words to look for in this conversation's transcript (all must match)."),
      limit: z.int().describe("Max entries to return (default 20).").optional(),
    }),
    async execute(callId, params) {
      const tc = ref.current;
      if (!tc) return text("[error] no active tool context");
  ...
  const background = defineTool({
    ...
    parameters: z.object({
      ...
      command: z.string().describe("start only: the shell command to run in the background.").optional(),
      process_id: z.string().describe("poll/write/stop/watch only: the id start returned.").optional(),
      data: z
        .string()
        .describe(
      ...
      instructions: z
        .string()
        .describe("watch only: a note replayed to you with each wake, e.g. 'summarize test failures only'.")
        .optional(),
      monitor_id: z.string().describe("unwatch only: the monitor id watch returned.").optional(),
      wait_seconds: z
        .int()
        .min(0)
        .max(60)
        .describe("poll only: block up to this long for new output or exit, then answer (default 0).")
        .optional(),
      max_bytes: z
        .int()
        .min(1)
        .describe("poll only: cap bytes this read (default 65536); re-poll with the returned cursor for more.")
        .optional(),
      signal: z
        .union([z.literal("TERM"), z.literal("KILL"), z.literal("INT"), z.literal("HUP"), z.literal("QUIT")])
        .describe("stop only: default TERM; use KILL to force.")
        .optional(),
      timeout_seconds: z
        .int()
        .min(1)
        .describe(`start only: hard lifetime in seconds before auto-stop (default ${bgTtlSec}, max ${bgTtlMaxSec}).`)
        .optional(),
    }),
    async execute(callId, params) {
      const tc = ref.current;
  ...
  const cron = defineTool({
    ...
    parameters: z.object({
      ...
      id: z.string().describe("get/runs/patch/delete/run/disable/retarget only: the cron id.").optional(),
      title: z
        .string()
        .describe(
      ...
      schedule: z
        .object({
          cron: z.string().describe("5-field cron expression: minute hour day-of-month month day-of-week.").optional(),
          timezone: z.string().describe('IANA timezone for a cron schedule, e.g. "America/Los_Angeles".').optional(),
          everyMs: z
            .int()
            .describe(
            ...
            .optional(),
          firstFireAt: z.int().describe("epoch ms for a one-shot (fires once, then auto-cancels).").optional(),
        })
        .describe("create/patch: when it runs — {cron,timezone} | {everyMs} | {firstFireAt}.")
        .optional(),
      task: z
        .string()
        .describe("create/patch: a prompt re-evaluated at fire time (a dynamic task). Use this OR text.")
        .optional(),
      text: z
        .string()
        .describe("create/patch: exact words to send verbatim (a reminder/relay). Use this OR task.")
        .optional(),
      recipient: z
        .string()
        .describe("create: a teammate's name to DM (core resolves it; you can't author a raw address).")
        .optional(),
      channel: z.string().describe("create: a channel name to post to (core resolves it).").optional(),
      scope: z
        .literal("personal")
        .describe(
      ...
      destinationKey: z
        .string()
        .describe('create/retarget: a key from the "Where scheduled tasks post" menu.')
        .optional(),
      unfurlLinks: z
        .boolean()
        .describe(
        ...
        .optional(),
      runAs: z
        .union([z.literal("owner"), z.literal("scopeFloor"), z.literal("scopeShared")])
        .describe(
        ...
        .optional(),
      enabled: z.boolean().describe("patch only: false pauses the cron, true resumes it.").optional(),
      archived: z.boolean().describe("patch only: true archives the cron.").optional(),
      limit: z
        .int()
        .min(1)
        .describe(
      ...
      offset: z
        .int()
        .min(0)
        .describe("list only: start the page here — pass the offset the previous page's footer names to continue.")
        .optional(),
    }),
    async execute(callId, params) {
      const tc = ref.current;
  ...
  const guidance = defineTool({
    ...
    parameters: z.object({
      action: z.union([z.literal("read"), z.literal("write")]),
      scope: z.union([z.literal("channel"), z.literal("conversation")]).optional(),
      content: z.string().optional(),
      bots: z
        .record(
          z.string(),
          z.object({
            mode: z.union(BOT_MODES.map((m) => z.literal(m))),
            rollupHours: z.number().optional(),
          }),
        )
        .describe(
  ...
  const share = defineTool({
    ...
    parameters: z.object({
      type: z
        .union([z.literal("file"), z.literal("skill"), z.literal("deploy"), z.literal("cron")])
        .describe("Which kind of artifact: file | skill | deploy | cron."),
      id: z.string().describe("The artifact's id (a deployment may also be named by its handle)."),
      toScope: z
        .string()
        .describe(
      ...
      permission: z
        .union([z.literal("read"), z.literal("write")])
        .describe("read = view/use (default), write = view + manage. Ignored for a move.")
        .optional(),
      move: z
        .boolean()
        .describe(
  ...
  const surface = defineTool({
    ...
    parameters: z.object({
      ...
      channel: z
        ...
        .optional(),
      recipient: z.string().describe("reach: DM this teammate (by name). NOT valid on post.").optional(),
      participants: z
        .array(z.string())
        .describe(
      ...
      files: z
        ...
        .optional(),
      emoji: z.string().describe('react: emoji name without colons (e.g. "eyes", "tada").').optional(),
      ref: z
        .string()
        .describe(
      ...
      limit: z
        ...
        .optional(),
      query: z.string().describe("search: words to look for in this place's messages.").optional(),
      source: z
        .union([z.literal("mirror"), z.literal("slack")])
        .describe(
...
function withToolApprovalGate(
  ...
  return {
    ...
    async execute(callId: string, params: unknown) {
      ...
      return inner(callId, params);
    },
  };
}

function withToolBodyTiming(tool: AnyToolDefinition, ref: ToolContextRef): AnyToolDefinition {
  const inner = tool.execute.bind(tool);
  return {
    ...tool,
    async execute(callId: string, params: unknown) {
      ...
      try {
        return await inner(callId, params);
      } finally {
        try {
