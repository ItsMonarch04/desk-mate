...
import { modelSupportedByHarness } from "../model/capabilities.ts";
import { DEFAULT_CODEX_MODEL_ID } from "../model/defaults.ts";
...
function toolOptions(opts: CodexHarnessOptions, turn?: HarnessTurnInput): CoreToolsOptions {
  return {
...
export function createCodexHarness(opts: CodexHarnessOptions = {}): Harness {
  ...
  const ensureRuntime = async (): Promise<Runtime> => {
    ...
    starting = (async () => {
      const jail = mkdtempSync(join(tmpdir(), "deskmate-codex-"));
  ...
  const runPrompt = async (turn: HarnessTurnInput, toolsEnabled = true): Promise<HarnessTurnResult> => {
    ...
    const threadStartRequest = {
      ...
      developerInstructions:
        "Use the supplied dynamic Deskmate tools for all workspace, execution, memory, history, and surface operations. The built-in working directory is an empty read-only control jail, not the user's workspace.",
      dynamicTools,
