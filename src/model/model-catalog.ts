import { modelSupportedByHarness } from "./capabilities.ts";
import { resolveModel, SELECTABLE_BASE_MODELS } from "./catalog.ts";
...
export function selectableCatalogForHarness(
  ...
  return catalog.filter(
    (model) => (model.provider !== "openrouter" || harness === "mock") && modelSupportedByHarness(model.id, harness),
  );
}
