...
function start(harnessId = "claude"): { base: string; built: BuiltApp; close: () => Promise<void> } {
...
test("branding governance validates, round-trips through surface-config, clears, and is org-admin-only", async () => {
  ...
  try {
    ...
    assert.equal(
      (
        await fetch(url, {
          method: "PUT",
          headers: ADMIN,
          body: JSON.stringify({ accent: "#6366f1", mark: "Q", selfLabel: "deskmate" }),
        })
      ).status,
      200,
    );
    ...
    assert.deepEqual(await surfaceBranding(), { accent: "#6366f1", mark: "Q", selfLabel: "deskmate" });
    assert.equal(
...
test("runtime-config lets a person set, keep, and inherit an approved personal runtime", async () => {
  const srv = start("claude");
  try {
    srv.built.config.setApprovedHarnesses(["claude", "codex", "claude"]);
    ...
    assert.equal(initial.effective.harnessId, "claude");
    ...
    assert.deepEqual(keptLegacy.scopeOverride, { harnessId: "claude", modelId: "claude-sonnet-4-6", orgRevision: 2 });
  } finally {
    await srv.close();
  }
});
