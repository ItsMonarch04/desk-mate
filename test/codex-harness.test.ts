...
import { DEFAULT_CODEX_MODEL_ID } from "../src/model/defaults.ts";
...
test("Codex harness drives app-server JSON-RPC with a read-only jail", async (t) => {
  const dir = mkdtempSync(join(tmpdir(), "deskmate-codex-test-"));
...
test("Codex materializes API-key auth into its isolated home, and never an ambient login", (t) => {
  const jail = mkdtempSync(join(tmpdir(), "deskmate-codex-auth-test-"));
  ...
  const bare = mkdtempSync(join(tmpdir(), "deskmate-codex-auth-bare-"));
...
test("Codex interrupts the provider after a terminal Deskmate tool", async (t) => {
  const dir = mkdtempSync(join(tmpdir(), "deskmate-codex-stop-test-"));
...
test("Codex spawn failure does not hang run or cleanup", async () => {
  const harness = createCodexHarness({ binaryPath: "/definitely/missing/deskmate-codex" });
  const scope = { kind: "org", id: "test" } as unknown as ScopeId;
...
test("Codex discards a nonresponsive startup so a later turn can retry", async (t) => {
  const dir = mkdtempSync(join(tmpdir(), "deskmate-codex-startup-test-"));
  const harness = createCodexHarness({
...
test("cancelling one Codex setup does not kill another active turn", async (t) => {
  const dir = mkdtempSync(join(tmpdir(), "deskmate-codex-concurrent-test-"));
  const harness = createCodexHarness({
...
for (const mode of ["turnFailed", "startRejected"] as const) {
  test(`Codex parks the run on a provider auth/quota failure (${mode}) instead of burning retries`, async (t) => {
    const dir = mkdtempSync(join(tmpdir(), "deskmate-codex-fail-test-"));
    const harness = createCodexHarness({
...
test("Codex records one llm row per turn carrying real timings and usage, even when the turn fails", async (t) => {
  const dir = mkdtempSync(join(tmpdir(), "deskmate-codex-telemetry-test-"));
...
test(
  ...
  async (t) => {
    const jail = mkdtempSync(join(tmpdir(), "deskmate-codex-real-"));
