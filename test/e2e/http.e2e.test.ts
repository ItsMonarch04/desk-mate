...
describe("HTTP e2e (live Pi over the API)", { skip: NO_KEY ? "set ANTHROPIC_API_KEY" : false }, () => {
  ...
  before(async () => {
    const built = buildApp(
      testConfig({
        ...
        harness: "claude",
        ...(process.env.AGENT_MODEL ? { modelId: process.env.AGENT_MODEL } : {}),
  ...
  it("write+read tools over HTTP (workspace)", { timeout: 120_000 }, async () => {
    const r = await turn(
      dm(
        "Use the write tool to create note.txt containing exactly: turn over http works. " +
          "Then read it back with the read tool and tell me the contents.",
      ),
    );
    assert.equal(r.http, 200);
    assert.match(r.json.reply, /turn over http works/i);
  });
