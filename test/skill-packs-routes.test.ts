...
function makeFixtureRepo(): { dir: string; sha: string } {
  const dir = mkdtempSync(join(tmpdir(), "deskmate-reg-fixture-"));
  const env = {
    ...process.env,
    GIT_AUTHOR_NAME: "t",
...
function makeRepoWithSharedLib(skillName: string): { dir: string; sha: string } {
  const dir = mkdtempSync(join(tmpdir(), "deskmate-col-fixture-"));
  const env = {
    ...process.env,
    GIT_AUTHOR_NAME: "t",
    GIT_AUTHOR_EMAIL: "t@t",
...
function makeRepoWithLayerCollision(): { dir: string; sha: string } {
  const dir = mkdtempSync(join(tmpdir(), "deskmate-layer-col-fixture-"));
  const env = {
    ...process.env,
    GIT_AUTHOR_NAME: "t",
    GIT_AUTHOR_EMAIL: "t@t",
...
test("re-import archives removed/renamed/now-ineligible skills and keeps unchanged ones", async () => {
  const dir = mkdtempSync(join(tmpdir(), "deskmate-recon-"));
  const env = {
    ...process.env,
    GIT_AUTHOR_NAME: "t",
    GIT_AUTHOR_EMAIL: "t@t",
...
test("sync refreshes IMPORTED skills (update + archive) but does NOT add un-imported ones; PATCH flips syncMode", async () => {
  const dir = mkdtempSync(join(tmpdir(), "deskmate-sync-"));
  const env = {
    ...process.env,
    GIT_AUTHOR_NAME: "t",
    GIT_AUTHOR_EMAIL: "t@t",
...
test("re-importing into ONE scope archives only that scope's deselected skills, not another scope's", async () => {
  const dir = mkdtempSync(join(tmpdir(), "deskmate-scoped-"));
  const env = {
    ...process.env,
    GIT_AUTHOR_NAME: "t",
    GIT_AUTHOR_EMAIL: "t@t",
...
test("sync refreshes EVERY scope the pack was imported into", async () => {
  const dir = mkdtempSync(join(tmpdir(), "deskmate-syncmulti-"));
  const env = {
    ...process.env,
    GIT_AUTHOR_NAME: "t",
    GIT_AUTHOR_EMAIL: "t@t",
...
test("a pack skill whose name collides with a native skill in ANOTHER scope still imports into its own scope", async () => {
  const dir = mkdtempSync(join(tmpdir(), "deskmate-xscope-"));
  const env = {
    ...process.env,
    GIT_AUTHOR_NAME: "t",
    GIT_AUTHOR_EMAIL: "t@t",
