...
const erroredAssistant = (error: string) => `{
  info: {
    ...
    error: ${error},
    parentID: "", modelID: "gpt-5", providerID: "openai", mode: "deskmate", path: { cwd: "/", root: "/" },
...
test("OpenCode surfaces a provider error as a non-retryable failure, never a successful empty reply", async (t) => {
  const dir = mkdtempSync(join(tmpdir(), "deskmate-opencode-test-"));
...
test("OpenCode startup failure reports the sidecar's real output and honors the configured timeout", async (t) => {
  ...
  // Generous: this case races the child's stderr against the deadline, and a loaded
  // CI runner can take longer than a few hundred ms just to spawn the shell.
  const noisyHarness = createOpenCodeHarness({ binaryPath: noisy, startupTimeoutMs: 5_000 });
