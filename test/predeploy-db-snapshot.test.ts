...
function envFor(fake: ReturnType<typeof setupAws>, extra: NodeJS.ProcessEnv = {}): NodeJS.ProcessEnv {
  return {
    ...
    PREDEPLOY_DB_INSTANCE: "deskmate-prod-core",
    PREDEPLOY_SNAPSHOT_ID: "test-snapshot",
    GITHUB_RUN_ID: "123",
...
test("creates and waits for a predeploy snapshot after checking retention", () => {
  ...
  assert.match(calls, /describe-db-instances --db-instance-identifier deskmate-prod-core/);
  assert.match(
    calls,
    /create-db-snapshot --db-instance-identifier deskmate-prod-core --db-snapshot-identifier test-snapshot/,
  );
...
test("keeps only the newest configured number of predeploy snapshots", () => {
  ...
  const calls = readFileSync(fake.log, "utf8");
  assert.match(calls, /starts_with\(DBSnapshotIdentifier, `deskmate-prod-core-predeploy-`\)/);
