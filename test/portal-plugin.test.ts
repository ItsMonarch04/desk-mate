...
test("portal proxy omits empty URL ports so http.request uses protocol defaults", () => {
  assert.equal(requestPort(new URL("http://deskmate-web-ui.flycast")), undefined);
  assert.equal(requestPort(new URL("https://deskmate-web-ui.flycast")), undefined);
  assert.equal(requestPort(new URL("http://deskmate-web-ui.flycast:8080")), "8080");
});
