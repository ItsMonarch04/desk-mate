...
import { DEFAULT_AGENT_MODEL_ID } from "../src/model/defaults.ts";

const read = (path: string) => readFileSync(new URL(`../${path}`, import.meta.url), "utf8");
const readme = read("README.md");
const index = read("src/index.ts");
const server = read("src/api/server.ts");
const piTools = read("src/harness/core-tools.ts");
...
test(".env.example does not pin a base model that drifts from the shipped default", () => {
  ...
  const pinned = /^AGENT_MODEL=(.+)$/m.exec(envExample)?.[1];
  if (pinned) assert.equal(pinned, DEFAULT_AGENT_MODEL_ID, "an active AGENT_MODEL pin must match the shipped default");
});
