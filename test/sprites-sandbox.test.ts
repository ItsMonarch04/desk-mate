...
test("scope name is stable and slugged", () => {
  const a = spriteScopeName("deskmatet", "person:tester");
  assert.equal(a, spriteScopeName("deskmatet", "person:tester"));
  assert.match(a, /^deskmatet-person-tester-[0-9a-f]{6}$/);
});
