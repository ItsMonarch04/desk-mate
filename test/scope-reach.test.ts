...
import { createCoreTools, type ToolContextRef } from "../src/harness/core-tools.ts";
import {
  createToolContext,
  type ToolContext,
...
const call = (tool: ReturnType<typeof createCoreTools>[number] | undefined, params: unknown) => {
  assert.ok(tool);
  return (tool.execute as unknown as (id: string, p: unknown) => Promise<unknown>)("t", params);
};
const schemaProps = (tool: ReturnType<typeof createCoreTools>[number]): string[] =>
  Object.keys((tool.jsonSchema as { properties: Record<string, unknown> }).properties);

test("reachExec OFF: the execute scope never accepts a room", () => {
  const ref: ToolContextRef = { current: null };
  const [legacy] = createCoreTools(ref);
  assert.ok(!schemaProps(legacy!).includes("scope"), "legacy surface has no scope param");
  const [scratchOnly] = createCoreTools(ref, { scratchExec: true });
  const scopeSchema = (scratchOnly!.jsonSchema as { properties: { scope?: { anyOf?: unknown[] } } }).properties.scope;
  assert.ok(
...
test("reachExec ON (no scratch): scope is a free string; a room routes to reachTarget; default is scoped", async () => {
  ...
  const [execute] = createCoreTools({ current: tc }, { reachExec: true });
...
test("reachExec ON + scratchExec ON: scope accepts scoped, scratch, AND a room", async () => {
  const { tc, seen } = sinkToolContext();
  const [execute] = createCoreTools({ current: tc }, { reachExec: true, scratchExec: true });
...
test("Trap 1: a reach result prefixes provenance and keeps the SESSION scope label, not the channel", async () => {
  ...
  const [execute] = createCoreTools(ref, { reachExec: true });
...
test("DM + directory + flag: execute(scope:#room) runs on that channel's own computer", async () => {
  ...
  assert.ok(
    fakeSprites.calls.some((c) => c.method === "POST" && /\/sprites\/deskmate-channel-c-ph-[^/]+\/exec$/.test(c.path)),
...
test("reach teardown destroys a visited room that has no computer of its own (no leaked box)", async () => {
  ...
  assert.equal(
    fakeSprites.names().some((n) => n.startsWith("deskmate-channel-c-ph-")),
    false,
