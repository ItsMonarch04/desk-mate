...
test("planTapeSeed serves a clean fold only in serve mode, falls back on defects", () => {
  ...
  const cleanRows = clean.map((r) => ({ ...r, harness: r.kind === "message" ? "claude" : undefined })) as TapeRecord[];
  assert.equal(planTapeSeed(cleanRows, "claude", "shadow").seed, null, "shadow never serves");
  const served = planTapeSeed(cleanRows, "claude", "serve");
  ...
  assert.equal(planTapeSeed(dangling, "claude", "serve").seed, null, "lint failure falls back");
  ...
  const skipped = planTapeSeed(foreign, "claude", "serve");
...
test("interrupt heal converts a heal-able tape into a servable one", () => {
  ...
  const plan = planTapeSeed(rows, "claude", "serve");
...
test("image artifact refs rehydrate into a servable fold without mutating tape rows", async () => {
  ...
  const rows = [
    ...
    assistant([{ type: "text", text: "seen" }], { harness: "claude" }),
    turnEnd(1),
  ];
  let loads = 0;
  ...
  assert.ok(planTapeSeed(rows, "claude", "serve", hydrated).seed, "rehydrated image history serves");
...
test("image rehydration memoizes storage reads but charges duplicate blocks to the byte budget", async () => {
  ...
  assert.notEqual(planTapeSeed(rows, "claude", "serve", hydrated).seed, null, "a budget-evicted fold still serves");
});
...
test("an image larger than the REMAINING budget is evicted to a placeholder, not left byteless", async () => {
  ...
  assert.notEqual(planTapeSeed(rows, "claude", "serve", hydrated).seed, null, "fold still serves");
});
...
test("image rehydration leaves missing artifacts fail-closed", async () => {
  ...
  assert.equal(planTapeSeed(rows, "claude", "serve", hydrated).seed, null);
});
...
test("compacted-away image refs consume no reads or hydration budget", async () => {
  ...
  const rows = [
    ...
    assistant([{ type: "text", text: "old answer" }], { harness: "claude" }),
    turnEnd(1),
    row({
      kind: "message",
      harness: "claude",
    ...
    assistant([{ type: "text", text: "new answer" }], { harness: "claude" }),
    turnEnd(3),
  ...
  assert.ok(planTapeSeed(rows, "claude", "serve", hydrated).seed);
});
...
test("dangling calls heal only after every image is rehydrated", async () => {
  ...
  const rows = [
    ...
    assistant([{ type: "toolCall", id: "c1", name: "exec", arguments: {} }], { harness: "claude" }),
  ];
