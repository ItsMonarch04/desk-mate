...
function start(
  ...
  const server = createInsecureTestServer(built.app, {
    ...
    modelCredentialFetch,
    harnessId: config.harness ?? "claude",
    providerKeys: {
...
test("admin model credentials are encrypted, write-only, live, and removable", async () => {
  const srv = start({ anthropicApiKey: "deployment-anthropic-key", harness: "opencode" });
  try {
...
test("OpenRouter catalog exposes runtime-supported tool models as selectable base models", async () => {
  ...
  const srv = start({ openrouterApiKey: "deployment-openrouter-key", harness: "mock" }, async (input) => {
    requested = String(input);
    catalogRequests++;
    return Response.json({
      data: [
        ...
        {
          id: "vendor/released-after-this-build",
          name: "Future Model",
          supported_parameters: ["tools"],
        },
        {
          id: "not-a-routed-slug",
          name: "Malformed",
          supported_parameters: ["tools"],
        },
        {
  ...
  try {
    ...
    assert.deepEqual(
      ...
      [
        ...
        { id: "vendor/released-after-this-build", name: "Future Model", provider: "openrouter" },
      ],
      "OpenRouter's own catalog is the authority on what it serves: a model newer than this " +
        "build is offered, one without tool support or without a routed `vendor/model` id is not",
    );
    ...
    assert.ok(governanceBody.modelsByHarness.mock!.some((model) => model.id === "anthropic/claude-sonnet-4.5"));
    ...
    assert.ok(runtimeBody.modelsByHarness.mock!.includes("anthropic/claude-sonnet-4.5"));
    assert.deepEqual(runtimeBody.modelCatalog["anthropic/claude-sonnet-4.5"], {
      name: "Anthropic: Claude Sonnet 4.5",
      provider: "openrouter",
    });
    assert.deepEqual(runtimeBody.effective, { harnessId: "mock", modelId: "anthropic/claude-sonnet-4.5" });
...
test("web turns keep a persisted OpenRouter model enabled when the refreshed catalog omits it", async () => {
  const srv = start({ openrouterApiKey: "deployment-openrouter-key", harness: "mock" }, async () =>
    Response.json({
      data: [
        {
...
test("managed provider keys do not advertise unsupported OpenCode or browser credentials", async () => {
...
test("web turns gate the requested and scope-selected harness against its real key source", async () => {
  ...
  try {
    ...
    srv.built.config.setApprovedHarnesses(["mock", "opencode"]);
