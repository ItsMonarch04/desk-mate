...
function tmpStore(): string {
  const store = mkdtempSync(join(tmpdir(), "deskmate-dev-test-"));
  ensureStore(store);
  return store;
}
...
test("meta round-trips and lease staleness follows pids + heartbeat", () => {
  ...
  const worktree = mkdtempSync(join(tmpdir(), "deskmate-wt-"));
...
test("env assembly precedence: caller > login shell > dev.env > worktree .env; harness gates", async () => {
  ...
  const prevLive = process.env.DESKMATE_DEV_ENV;
  process.env.DESKMATE_DEV_ENV = liveEnv;
  const noise: string[] = [];
  ...
  assert.equal(fromCaller.harness, "claude");
  assert.equal(fromCaller.env.HARNESS, "claude");
  ...
  if (prevLive === undefined) delete process.env.DESKMATE_DEV_ENV;
  else process.env.DESKMATE_DEV_ENV = prevLive;
  rmSync(worktree, { recursive: true, force: true });
});
...
test("OpenCode config is strict, pinned, and inherits the agent model", () => {
  assert.equal(OPENCODE_RUNTIME_VERSION, "1.17.18");
  assert.equal(loadConfig({ HARNESS: "opencode", AGENT_MODEL: "agent-model" }).opencodeModel, "agent-model");
  assert.equal(
    loadConfig({ HARNESS: "opencode", AGENT_MODEL: "agent-model", OPENCODE_MODEL: "open-model" }).opencodeModel,
    "open-model",
  );
  ...
  assert.throws(() => loadConfig({ HARNESS: "bogus" }), /use mock, opencode, codex, or claude/);
  assert.throws(() => loadConfig({ HARNESS: "CLAUDE" }), /use mock, opencode, codex, or claude/);
});
