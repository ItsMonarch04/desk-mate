...
test("the security screen proxy sends the neutral classifier contract and maps its scores", async () => {
  ...
  assert.deepEqual(JSON.parse(String(calls[0]!.init.body)), {
    ...
    metadata: {
      origin: "automation",
      surface: "webhook",
      deskmate: { request_id: "request-1", input_index: 0, chunk_index: 0, chunk_count: 1 },
...
test("the security screen proxy classifies long input in overlapping bounded windows", async () => {
  const bodies: Array<{
    text: string;
    metadata: { deskmate: { request_id: string; input_index: number; chunk_index: number; chunk_count: number } };
  }> = [];
  let active = 0;
  let maxActive = 0;
  const screener = createSecurityScreenProxy({
  ...
  const ordered = bodies.toSorted((a, b) => a.metadata.deskmate.chunk_index - b.metadata.deskmate.chunk_index);
  assert.equal(ordered[0]!.text.slice(-256), ordered[1]!.text.slice(0, 256));
  assert.ok(
    ordered.every(
      (body, index) =>
        body.metadata.deskmate.request_id === ordered[0]!.metadata.deskmate.request_id &&
        body.metadata.deskmate.input_index === 0 &&
        body.metadata.deskmate.chunk_index === index &&
        body.metadata.deskmate.chunk_count === 12,
    ),
  );
  assert.ok(
    ordered.every(
      (body) =>
        ...
        JSON.stringify(body.metadata.deskmate),
    ),
  );
});
...
test("the security screen proxy aborts sibling chunks after a terminal failure", async () => {
  ...
  const screener = createSecurityScreenProxy({
    ...
    shadow: false,
    fetch: async (_input, init) => {
      const body = JSON.parse(String(init?.body)) as { metadata: { deskmate: { chunk_index: number } } };
      const index = body.metadata.deskmate.chunk_index;
      started.push(index);
