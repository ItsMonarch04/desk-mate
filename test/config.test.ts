...
test("ORG_BRAND_* parses into a validated branding default", () => {
  ...
  assert.deepEqual(
    loadConfig({ ORG_BRAND_ACCENT: "#6366f1", ORG_BRAND_MARK: "Q", ORG_BRAND_SELF_LABEL: "deskmate" }).brandingDefault,
    { accent: "#6366f1", mark: "Q", selfLabel: "deskmate" },
  );
...
test("production names a mock harness rather than letting it pass as a real deployment", () => {
  ...
  try {
    ...
    loadConfig({ ...productionEnv, HARNESS: "claude" });
    loadConfig({});
  } finally {
...
test("every boolean knob accepts the shared vocabulary (off means off)", () => {
  const off = loadConfig({ SEED_SKILLS: "off", EXECUTE_SCRATCH: "off", REACH_EXEC: "off" });
  ...
  assert.equal(unset.seedSkills, true, "seeding defaults on");
  assert.equal(unset.scratchExecEnabled, false, "scratch exec defaults off");
});
...
test("HARNESS=claude can boot before an admin configures a model provider", () => {
  assert.doesNotThrow(() => loadConfig({ HARNESS: "claude" }));
  assert.doesNotThrow(() => loadConfig({ HARNESS: "claude", ANTHROPIC_API_KEY: "sk-ant" }));
  assert.doesNotThrow(() => loadConfig({ HARNESS: " claude " }));
  assert.doesNotThrow(() => loadConfig({ HARNESS: " claude ", ANTHROPIC_API_KEY: "sk-ant" }));
  assert.doesNotThrow(() => loadConfig({ HARNESS: "mock" }));
  assert.doesNotThrow(() => loadConfig({ ...productionEnv, HARNESS: "claude" }));
  assert.doesNotThrow(() => loadConfig({ ...productionEnv, HARNESS: "claude", ANTHROPIC_API_KEY: "sk-ant" }));
});
...
test("Fly identity and Slack runtime settings are parsed once into Config", () => {
  const config = loadConfig({
    FLY_APP_NAME: "deskmate-core",
    SLACK_BOT_TOKEN: "xoxb-test",
    SLACK_APP_TOKEN: "xapp-test",
    SLACK_API_URL: "https://slack.example/api",
  });
  assert.equal(config.flyAppName, "deskmate-core");
