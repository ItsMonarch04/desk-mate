...
// provisioning schema (cli/src/secrets.ts, driving `deskmate secrets`) are maintained separately by
...
test("runtime schema conditions reference env vars the CLI schema also conditions on", () => {
  ...
  // so `deskmate secrets` and core boot validation agree about when a secret becomes required.
