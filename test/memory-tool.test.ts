...
test("createCoreTools exposes ONE `memory` tool: search returns scope-keyed hits + emits a trace", async () => {
  const workspace = createLocalWorkspaceStore(mkdtempSync(join(tmpdir(), "ws-recall-")));
  const memory = createMemoryService(workspace);
  const personal = scopeId("personal", "U1");
  await workspace.ensureScope(personal);
  ...
  const tools = createCoreTools(ref);
  assert.equal(
...
test("the `memory` tool remember/read/rewrite round-trip through the durable service", async () => {
  const workspace = createLocalWorkspaceStore(mkdtempSync(join(tmpdir(), "ws-mem-")));
  const memory = createMemoryService(workspace);
  const personal = scopeId("personal", "U1");
  await workspace.ensureScope(personal);
  const ref: ToolContextRef = {
  ...
  const memoryTool = createCoreTools(ref).find((t) => t.name === "memory");
...
test("a read-only wake keeps `memory` but refuses its write actions", async () => {
  ...
  const memoryTool = createCoreTools(ref, { readOnly: true }).find((t) => t.name === "memory");
...
test("the `memory` tool params expose NO scope field — the model cannot redirect the notebook", () => {
  const memoryTool = createCoreTools({ current: null }).find((t) => t.name === "memory");
  assert.ok(memoryTool);
  const props = (memoryTool.jsonSchema as { properties?: Record<string, unknown> }).properties ?? {};
